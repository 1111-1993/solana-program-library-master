...
pub fn init_system_account<'a, 'b>(
    ...
    if !target_account.data_is_empty() || target_account.try_weis()? != 0 {
        return Ok(());
    }
...
pub fn check_pda_rent_exempt<'a, 'b>(
    ...
    let rent = Rent::get()?;
    let cur_balance = target_account.try_weis()?;
    let min_balance = rent.minimum_balance(data_size);
    if cur_balance < min_balance {
        let signer_balance = signer_account.try_weis()?;
        ...
        assert!(target_account.try_weis()? >= min_balance);
    }
    Ok(())
}
