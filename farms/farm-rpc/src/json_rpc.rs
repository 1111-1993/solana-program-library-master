...
#[post("/create_system_account?<wallet_keypair>&<new_account_keypair>&<weis>&<space>&<owner>")]
...
#[post("/transfer?<wallet_keypair>&<destination_wallet>&<gth_ui_amount>")]
async fn transfer(
    wallet_keypair: Option<KeypairParam>,
    ...
    let farm_client = farm_client
        ...
        .map_err(|e| NotFound(e.to_string()))?;
    let signature = farm_client
        .transfer(&wallet_keypair, &destination_wallet, gth_ui_amount)
        .map_err(|e| NotFound(e.to_string()))?;
...
#[post("/transfer_gth_to_wsol?<wallet_keypair>&<gth_ui_amount>")]
async fn transfer_gth_to_wsol(
    wallet_keypair: Option<KeypairParam>,
    gth_ui_amount: f64,
    farm_client: &State<FarmClientArc>,
) -> Result<String, NotFound<String>> {
    ...
    let farm_client = farm_client
        ...
        .map_err(|e| NotFound(e.to_string()))?;
    let signature = farm_client
        .transfer_gth_to_wsol(&wallet_keypair, gth_ui_amount)
        .map_err(|e| NotFound(e.to_string()))?;
...
#[get("/new_instruction_create_system_account?<wallet_address>&<new_address>&<weis>&<space>&<owner>")]
...
/// Creates the native GTH transfer instruction
#[get("/new_instruction_transfer?<wallet_address>&<destination_wallet>&<gth_ui_amount>")]
async fn new_instruction_transfer(
    ...
    let farm_client = farm_client
        ...
        .map_err(|e| NotFound(e.to_string()))?;
    let instruction = farm_client
        .new_instruction_transfer(&wallet_address, &destination_wallet, gth_ui_amount)
        .map_err(|e| NotFound(e.to_string()))?;
