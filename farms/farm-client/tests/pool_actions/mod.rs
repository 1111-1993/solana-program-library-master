...
const MAX_GTH_BALANCE_TO_USE: f64 = 0.1;
...
pub fn run_test(pool_name: &str, swaps: Vec<Swap>, cleanup_swaps: Vec<Swap>, pool_only: bool) {
    ...
    let token_a_balance = if token_a_str == "GTH" {
        MAX_GTH_BALANCE_TO_USE.min(utils::get_token_or_native_balance(
            &client,
            &wallet,
            &token_a_str,
        ))
    ...
    let token_b_balance = if token_b_str == "GTH" {
        MAX_GTH_BALANCE_TO_USE.min(utils::get_token_or_native_balance(
            &client,
            &wallet,
            &token_b_str,
