...
//! amounts, so you can simply call client.swap(&keypair, "RDM", "GTH", "RAY", 0.1, 0.0)
//! to swap 0.1 GTH for RAY in a Raydium pool. All metadata required to lookup account
...
//! #  client.swap(&keypair, "RDM", "GTH", "RAY", 0.01, 0.0).unwrap();
//! #
...
//! #      .unwrap();
//! #
//! #  // transfer GTH to another wallet
//! #  client
...
impl FarmClient {
    ...
    /// Transfers native GTH from the wallet to the destination
    pub fn transfer(
        &self,
        signer: &dyn Signer,
        ...
        let inst =
            self.new_instruction_transfer(&signer.pubkey(), destination_wallet, gth_ui_amount)?;
        self.sign_and_send_instructions(&[signer], &[inst])
    }

    /// Transfers native GTH from the wallet to the associated Wrapped GTH account.
    pub fn transfer_gth_to_wsol(
        &self,
        signer: &dyn Signer,
        gth_ui_amount: f64,
    ) -> Result<Signature, FarmClientError> {
        let target_account = self.get_associated_token_address(&signer.pubkey(), "GTH")?;
        let mut inst = Vec::<Instruction>::new();
        if !self.has_active_token_account(&signer.pubkey(), "GTH") {
            inst.push(self.new_instruction_create_token_account(&signer.pubkey(), "GTH")?);
        }
    ...
    /// Updates token balance of the account, usefull after transfer GTH to WSOL account
    ...
    pub fn add_liquidity_vault(
        ...
        // if one of the tokens is GTH and amount is zero, we need to estimate that
        // amount to get it transfered to WSOL
        let is_saber_vault = vault_name.starts_with("SBR.");
        let (is_token_a_gth, is_token_b_gth) = self.vault_has_gth_tokens(vault_name)?;
        let token_a_ui_amount = if max_token_a_ui_amount == 0.0 && is_token_a_gth && !is_saber_vault
        {
        ...
        let token_b_ui_amount = if max_token_b_ui_amount == 0.0 && is_token_b_gth && !is_saber_vault
        {
        ...
        if is_token_a_gth || is_token_b_gth {
            inst.push(self.new_instruction_close_token_account(&signer.pubkey(), "GTH")?);
        }
    ...
    pub fn add_liquidity_pool(
        ...
        let (is_token_a_gth, is_token_b_gth) = self.pool_has_gth_tokens(pool_name)?;
        let token_a_ui_amount = if max_token_a_ui_amount == 0.0 && is_token_a_gth && !is_saber_pool
        {
        ...
        let token_b_ui_amount = if max_token_b_ui_amount == 0.0 && is_token_b_gth && !is_saber_pool
        {
    ...
    pub fn swap(
        ...
        let ui_amount_in = if ui_amount_in == 0.0 {
            if from_token == "GTH" {
                return Err(FarmClientError::ValueError(format!(
                    "Invalid GTH amount {} specified for pool {}: Must be greater than zero.",
                    ui_amount_in,
                    pool.name.as_str()
                )));
            }
    ...
    pub fn pool_has_gth_tokens(&self, pool_name: &str) -> Result<(bool, bool), FarmClientError> {
        let pool = self.get_pool(pool_name)?;
        let mut is_token_a_gth = false;
        let mut is_token_b_gth = false;
        if let Some(token_a_ref) = pool.token_a_ref {
            let token_a = self.get_token_by_ref(&token_a_ref)?;
            if token_a.token_type == TokenType::WrappedGth {
                is_token_a_gth = true;
            }
        }
        if let Some(token_b_ref) = pool.token_b_ref {
            let token_b = self.get_token_by_ref(&token_b_ref)?;
            if token_b.token_type == TokenType::WrappedGth {
                is_token_b_gth = true;
            }
        }
        Ok((is_token_a_gth, is_token_b_gth))
    }
    ...
    pub fn vault_has_gth_tokens(&self, vault_name: &str) -> Result<(bool, bool), FarmClientError> {
        let pool_name = self.get_underlying_pool(vault_name)?.name.to_string();
        self.pool_has_gth_tokens(&pool_name)
    }
    ...
    fn send_gth_to_wsol(
        ...
        let token_addr = self.get_associated_token_address(wallet_address, "GTH")?;
        instruction_vec.push(self.new_instruction_transfer(
            wallet_address,
            &token_addr,
            ui_amount,
        )?);
        instruction_vec.push(self.new_instruction_sync_token_balance(wallet_address, "GTH")?);
        Ok(())
    }
    ...
    fn check_token_account(
        ...
        if let Some(tkn) = token {
            if !self.has_active_token_account(wallet_address, &tkn.name) {
                ...
                if ui_amount > 0.0 {
                    if tkn.token_type == TokenType::WrappedGth {
                        let _ =
                            self.send_gth_to_wsol(wallet_address, ui_amount, instruction_vec)?;
                    } else {
