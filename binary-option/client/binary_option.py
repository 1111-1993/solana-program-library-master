        ...
        try:
            ...
            try:
                if amount is None:
                    ...
                    weis = min_rent_reseponse["result"]
                else:
                    weis = int(amount)
                msg += f" | Fetched weis: {weis * 1e-9} GTH"
            except Exception as e:
                msg += " | ERROR: couldn't process weis" 
                raise(e)
            # Generate transaction
            transfer_ix = transfer(TransferParams(from_pubkey=sender_account.public_key(), to_pubkey=dest_account, weis=weis))
            ...
            try:
                ...
                return json.dumps(
                    {
                        'status': HTTPStatus.OK,
                        'msg': f"Successfully sent {weis * 1e-9} GTH to {to}",
                        'tx': response.get('result') if skip_confirmation else response['result']['transaction']['signatures'],
                    }
                )
            except Exception as e:
