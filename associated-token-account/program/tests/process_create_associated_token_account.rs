...
async fn test_associated_token_address() {
    ...
    assert_eq!(associated_account.weis, expected_token_account_balance);
}

#[tokio::test]
async fn test_create_with_fewer_weis() {
    let wallet_address = Pubkey::new_unique();
    let token_mint_address = Pubkey::new_unique();
    let associated_token_address = get_associated_token_address_with_program_id(
        &wallet_address,
    ...
    // Transfer weis into `associated_token_address` before creating it - enough to be
    ...
    // Check that the program adds the extra weis
    let mut transaction = Transaction::new_with_payer(
        &[create_associated_token_account(
            &payer.pubkey(),
            &wallet_address,
...
#[tokio::test]
async fn test_create_with_excess_weis() {
    let wallet_address = Pubkey::new_unique();
    let token_mint_address = Pubkey::new_unique();
    let associated_token_address = get_associated_token_address_with_program_id(
        &wallet_address,
    ...
    // Transfer 1 wei into `associated_token_address` before creating it
    let mut transaction = Transaction::new_with_payer(
        &[system_instruction::transfer(
            &payer.pubkey(),
            &associated_token_address,
    ...
    // Check that the program doesn't add any weis
    let mut transaction = Transaction::new_with_payer(
        &[create_associated_token_account(
            &payer.pubkey(),
            &wallet_address,
