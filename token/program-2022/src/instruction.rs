...
pub enum TokenInstruction<'a> {
    ...
    /// will be equal to the amount of GTH in the account. If this account is
    ...
    /// amounts of GTH and Tokens will be transferred to the destination
    /// account.
    ///
    ...
    /// Close an account by transferring all its GTH to the destination account.
    ...
    /// Given a wrapped / native token account (a token account containing GTH)
    /// updates its amount field based on the account's underlying `weis`.
    /// This is useful if a non-wrapped GTH account uses `system_instruction::transfer`
    /// to move weis to a wrapped token account, and needs to have its token
    /// `amount` field updated.
    ///
    /// Accounts expected by this instruction:
    ///
    ///   0. `[writable]`  The native token account to sync with its underlying weis.
    SyncNative,
    ...
    /// `u64`.
    ///
    /// Return data can be fetched using `gth_get_return_data` and deserializing
    ...
    ///
    /// Return data can be fetched using `gth_get_return_data` and deserialized with
    /// `String::from_utf8`.
    ///
    /// Accounts expected by this instruction:
    ///
    ...
    /// Wrapped GTH (`native_mint::id()`) will not be available until this instruction is
