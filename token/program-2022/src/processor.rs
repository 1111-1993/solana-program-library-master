...
impl Processor {
    fn _process_initialize_mint(
        ...
        if !rent.is_exempt(mint_info.weis(), mint_data_len) {
    ...
    fn _process_initialize_account(
        ...
        if !rent.is_exempt(new_account_info.weis(), new_account_info_data_len) {
            return Err(TokenError::NotRentExempt.into());
        }
    ...
    fn _process_initialize_multisig(
        ...
        if !rent.is_exempt(multisig_info.weis(), multisig_info_data_len) {
            return Err(TokenError::NotRentExempt.into());
        }
    ...
    pub fn process_transfer(
        ...
        if source_account.base.is_native() {
            let source_starting_weis = source_account_info.weis();
            **source_account_info.weis.borrow_mut() = source_starting_weis
                .checked_sub(amount)
                .ok_or(TokenError::Overflow)?;

            let destination_starting_weis = destination_account_info.weis();
            **destination_account_info.weis.borrow_mut() = destination_starting_weis
                .checked_add(amount)
                .ok_or(TokenError::Overflow)?;
        }
    ...
    pub fn process_close_account(program_id: &Pubkey, accounts: &[AccountInfo]) -> ProgramResult {
        ...
        let destination_starting_weis = destination_account_info.weis();
        **destination_account_info.weis.borrow_mut() = destination_starting_weis
            .checked_add(source_account_info.weis())
            .ok_or(TokenError::Overflow)?;

        **source_account_info.weis.borrow_mut() = 0;
        let data_len = source_account_data.len();
        gth_memset(*source_account_data, 0, data_len);
    ...
    pub fn process_create_native_mint(accounts: &[AccountInfo]) -> ProgramResult {
        ...
        let weis_diff = new_minimum_balance.saturating_sub(native_mint_info.weis());
        invoke(
            &system_instruction::transfer(payer_info.key, native_mint_info.key, weis_diff),
            &[
                payer_info.clone(),
...
impl PrintProgramError for TokenError {
    ...
    {
        match self {
            TokenError::NotRentExempt => msg!("Error: Wei balance below rent-exempt threshold"),
...
mod tests {
    ...
    impl solana_sdk::program_stubs::SyscallStubs for SyscallStubs {
        fn gth_log(&self, _message: &str) {}

        fn gth_invoke_signed(
            &self,
        ...
        fn gth_get_clock_sysvar(&self, var_addr: *mut u8) -> u64 {
            unsafe {
        ...
        fn gth_get_epoch_schedule_sysvar(&self, _var_addr: *mut u8) -> u64 {
            program_error::UNSUPPORTED_SYSVAR
        }

        #[allow(deprecated)]
        fn gth_get_fees_sysvar(&self, _var_addr: *mut u8) -> u64 {
            program_error::UNSUPPORTED_SYSVAR
        }

        fn gth_get_rent_sysvar(&self, var_addr: *mut u8) -> u64 {
            unsafe {
        ...
        fn gth_set_return_data(&self, data: &[u8]) {
    ...
    fn test_initialize_mint() {
        ...
        mint_account.weis = mint_minimum_balance();
    ...
    fn test_initialize_mint_account() {
        ...
        account_account.weis = account_minimum_balance();
    ...
    fn test_multisig() {
        ...
        multisig_account.weis = multisig_minimum_balance();
    ...
    fn test_close_account() {
        ...
        assert_eq!(account_account.weis, account_minimum_balance());
        ...
        .unwrap();
        assert_eq!(account_account.weis, 0);
        assert_eq!(account3_account.weis, 2 * account_minimum_balance());
        let account = Account::unpack_unchecked(&account_account.data).unwrap();
        ...
        .unwrap();
        assert_eq!(account_account.weis, 0);
        assert_eq!(account3_account.weis, 2 * account_minimum_balance() + 2);
        let account = Account::unpack_unchecked(&account_account.data).unwrap();
        assert_eq!(account.amount, 0);
    ...
    fn test_native_token() {
        ...
        assert!(account.is_native());
        assert_eq!(account.amount, 0);
        assert_eq!(account2_account.weis, account_minimum_balance() + 40);
        let account = Account::unpack_unchecked(&account2_account.data).unwrap();
        assert!(account.is_native());
        assert_eq!(account.amount, 40);
    ...
    fn test_sync_native() {
        ...
        // transfer gth
        let new_weis = weis + 50;
        native_account.weis = account_minimum_balance() + new_weis;
        ...
        .unwrap();
        let account = Account::unpack_unchecked(&native_account.data).unwrap();
        assert_eq!(account.amount, new_weis);

        // reduce gth
        native_account.weis -= 1;
