...
use {
    ...
    solana_sdk::{
        message::Message, native_token::Gth, program_pack::Pack, pubkey::Pubkey, signature::Signer,
        system_instruction,
    },
...
fn command_create_accounts(
    ...
    for address_chunk in token_addresses_with_seed.chunks(100) {
        ...
        for (account, (address, seed)) in accounts_chunk.iter().zip(address_chunk) {
            if account.is_none() {
                weis_required += minimum_balance_for_rent_exemption;
    ...
    send_messages(config, &messages, weis_required, signers)
}
...
fn send_messages(
    ...
    crate::check_fee_payer_balance(config, weis_required)?;
