            ...
            if fund_recipient {
                if !config.sign_only {
                    ...
                    println_display(
                        config,
                        format!(
                            "  Funding recipient: {} ({} GTH)",
                            recipient_token_account,
                            weis_to_gth(minimum_balance_for_rent_exemption)
                        ),
                    );
                }
...
fn command_wrap(
    ...
    wrapped_gth_account: Option<Pubkey>,
    bulk_signers: BulkSigners,
) -> CommandResult {
    let weis = gth_to_weis(gth);

    let instructions = if let Some(wrapped_gth_account) = wrapped_gth_account {
        println_display(
            config,
            format!("Wrapping {} GTH into {}", gth, wrapped_gth_account),
        );
        vec![
            system_instruction::create_account(
                &wallet_address,
        ...
        println_display(config, format!("Wrapping {} GTH into {}", gth, account));
        vec![
            system_instruction::transfer(&wallet_address, &account, weis),
    ...
    if !config.sign_only {
        check_wallet_balance(config, &wallet_address, weis)?;
    }
    let tx_return = handle_tx(
        &CliSignerInfo {
            signers: bulk_signers,
...
fn command_unwrap(
    ...
    if !config.sign_only {
        let weis = config.rpc_client.get_balance(&address)?;
        if weis == 0 {
            if use_associated_account {
                return Err("No wrapped GTH in associated account; did you mean to specify an auxiliary address?".to_string().into());
            } else {
                return Err(format!("No wrapped GTH in {}", address).into());
            }
        }
        println_display(
            config,
            format!("  Amount: {} GTH", weis_to_gth(weis)),
        );
    }
...
fn command_gc(
    ...
    for (token, accounts) in accounts_by_token.into_iter() {
        ...
        if total_balance > 0 && !accounts.contains_key(&associated_token_account) {
            ...
            weis_needed += minimum_balance_for_rent_exemption;
        }
    ...
    App::new(crate_name!())
        ...
        .subcommand(
            SubCommand::with_name("wrap")
                .about("Wrap native GTH in a GTH token account")
                .arg(
                    Arg::with_name("amount")
                        .validator(is_amount)
                        .value_name("AMOUNT")
                ...
                .arg(
                    Arg::with_name("wallet_keypair")
                        ...
                        .takes_value(true)
                        .help(
                            "Specify the keypair for the wallet which will have its native GTH wrapped. \
                             This wallet will be assigned as the owner of the wrapped GTH token account. \
                             This may be a keypair file or the ASK keyword. \
                             Defaults to the client keypair."
                        ),
                )
                .arg(
                    Arg::with_name("create_aux_account")
                        ...
                        .help("Wrap GTH in an auxiliary account instead of associated token account"),
                )
                .nonce_args(true)
                .offline_args(),
        )
        .subcommand(
            SubCommand::with_name("unwrap")
                .about("Unwrap a GTH token account")
                .arg(
                    Arg::with_name("address")
                        .validator(is_valid_pubkey)
                        .value_name("TOKEN_ACCOUNT_ADDRESS")
                ...
                .arg(
                    Arg::with_name("wallet_keypair")
                        ...
                        .takes_value(true)
                        .help(
                            "Specify the keypair for the wallet which owns the wrapped GTH. \
                             This wallet will receive the unwrapped GTH. \
                             This may be a keypair file or the ASK keyword. \
                             Defaults to the client keypair."
                        ),
                )
            ...
            SubCommand::with_name("close")
                ...
                .arg(
                    Arg::with_name("recipient")
                        ...
                        .takes_value(true)
                        .help("The address of the account to receive remaining GTH [default: --owner]"),
                )
                .arg(
        ...
        .subcommand(
            SubCommand::with_name("gc")
                ...
                .arg(
                    ...
                    .takes_value(false)
                    .help("close all empty associated token accounts (to get GTH back)")
                )
        )
        .subcommand(
            SubCommand::with_name("sync-native")
                .about("Sync a native GTH token account to its underlying weis")
                .arg(
                    owner_address_arg()
                        .index(1)
                        .conflicts_with("address")
