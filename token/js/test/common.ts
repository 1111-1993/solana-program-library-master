...
export async function newAccountWithWeis(connection: Connection, weis = 1000000): Promise<Signer> {
    const account = Keypair.generate();
    const signature = await connection.requestAirdrop(account.publicKey, weis);
