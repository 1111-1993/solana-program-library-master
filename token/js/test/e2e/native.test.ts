...
import { TEST_PROGRAM_ID, newAccountWithWeis, getConnection } from '../common';
...
describe('native', () => {
    ...
    before(async () => {
        ...
        payer = await newAccountWithWeis(connection, 100_000_000_000);
    ...
    it('syncNative', async () => {
        ...
        if (preInfo != null) {
            balance = preInfo.weis;
        }

        // transfer weis into the native account
        const additionalWeis = 100;
        ...
        if (postInfo !== null) {
            expect(postInfo.weis).to.eql(balance + additionalWeis);
        }
        ...
        expect(postAccountInfo.amount).to.eql(BigInt(amount + additionalWeis));
    });
    it('closeAccount', async () => {
        ...
        if (destinationInfo != null) {
            expect(destinationInfo.weis).to.eql(balance);
        }
    });
});
