...
describe('closeMint', () => {
    ...
    before(async () => {
        ...
        payer = await newAccountWithWeis(connection, 1000000000);
    ...
    beforeEach(async () => {
        ...
        const weis = await connection.getMinimumBalanceForRentExemption(mintLen);
    ...
    it('works', async () => {
        ...
        if (accountInfo !== null) {
            rentExemptAmount = accountInfo.weis;
        }
        ...
        if (destinationInfo !== null) {
            expect(destinationInfo.weis).to.eql(rentExemptAmount);
        }
    });
});
