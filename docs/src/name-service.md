...
- Domain name declarations: One could arbitrarily set-up a class that we can call
  ...
  own the TLD `".gth"` and create a class of ".gth" sub-domains, administrating
  the issuance of the `"something.gth"` sub-domains that way (by setting the
  parent name to the address of the `".gth"` registry).
