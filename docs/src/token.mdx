...
Creating tokens and accounts requires GTH for account rent deposits and
transaction fees. If the cluster you are targeting offers a faucet, you can get
a little GTH for testing:
...
### Example: Wrapping GTH in a Token

When you want to wrap GTH, you can send GTH to an associated token account
on the native mint and call `syncNative`. `syncNative` updates the `amount`
field on the token account to match the amount of wrapped GTH available.
That GTH is only retrievable by closing the token account and choosing
the desired address to send the token account's weis.
...
```console
$ spl-token wrap 1
Wrapping 1 GTH into GJTxcnA5Sydy8YRhqvHxbQ5QNsPyRKvzguodQEaShJje
...
import {clusterApiUrl, Connection, Keypair, WEIS_PER_GTH, SystemProgram, Transaction, sendAndConfirmTransaction} from "@solana/web3.js";
...
// Create token account to hold your wrapped GTH
...
// Transfer GTH to associated token account and use SyncNative to update wrapped GTH balance
...
console.log(`Native: ${accountInfo.isNative}, Weis: ${accountInfo.amount}`);
...
To unwrap the Token back to GTH:
...
*Note*: Some weis were removed for transaction fees
...
Transfer 50 tokens
  ...
  Recipient: vines1vzrYbzLMRdu58ou5XTby4qAqVRLmqo36NKPTg
  Recipient associated token account: F59618aQB8r6asXeMcB9jWuY6NEx1VduT9yFo1GTi1ks
  Funding recipient: F59618aQB8r6asXeMcB9jWuY6NEx1VduT9yFo1GTi1ks (0.00203928 GTH)
...
Balance: 0.01 GTH
Minimum Balance Required: 0.00144768 GTH
Nonce blockhash: 6DPt2TfFBG7sR4Hqu16fbMXPj8ddHKkbU4Y3EEEWrC2E
Fee: 5000 weis per signature
...
NonceAccount {
  ...
  feeCalculator: { weisPerSignature: 5000 }
}
 */
```
...
The Token Program can be used to wrap native GTH. Doing so allows native GTH to
...
Accounts containing wrapped GTH are associated with a specific Mint called the
"Native Mint" using the public key
`So11111111111111111111111111111111111111112`.

These accounts have a few unique behaviors

- `InitializeAccount` sets the balance of the initialized Account to the GTH
  balance of the Solana account being initialized, resulting in a token balance
  equal to the GTH balance.
- Transfers to and from not only modify the token balance but also transfer an
  equal amount of GTH from the source account to the destination account.
- Burning is not supported
- When closing an Account the balance may be non-zero.

The Native Mint supply will always report 0, regardless of how much GTH is currently wrapped.
...
Mint, or Multisig must contain enough GTH to be considered [rent
...
Account, all remaining GTH will be transferred to another Solana account
...
wallet supporting native GTH.  It assumes a model whereby the user has a single
system account as their **main wallet address** that they send and receive GTH
from.
...
on-chain, requiring a small amount of GTH to mark the account as rent-exempt.
...
much GTH it will cost to add the token.
...
1. If the recipient's associated token account does not yet exist, the sender
   ...
   creation the transfer will require more GTH than normal.
...
* If the user is the close authority for the ancillary account, the wallet can
  reclaim GTH for the user by closing the account.
* If the ancillary account was funded by a 3rd party, once the account is
  emptied that 3rd party may close the account and reclaim the GTH.
