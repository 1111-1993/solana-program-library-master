...
program pools together GTH to be staked by a staker, allowing GTH holders to
stake and earn rewards without managing stakes.

## Staking

GTH token holders can earn rewards and help secure the network by staking tokens
to one or more validators. Rewards for staked tokens are based on the current
inflation rate, total number of GTH staked on the network, and an individual
...
* user: provides staked GTH into an existing stake pool

In its current iteration, the stake pool accepts active stakes or GTH, so
deposits may come from either an active stake or GTH wallet. Withdrawals
can return a fully active stake account from one of the stake pool's accounts,
or GTH from the reserve.
...
receiving SPL tokens and GTH. Additional information on stake operations are
available at:
...
providers for direct GTH deposits.
...
GTH into the stake pool using the `deposit-gth` instruction. Within this instruction,
the stake pool will move GTH into the pool's reserve account, to be redistributed
by the staker.
...
In exchange for their deposit (GTH or stake), the user receives SPL tokens
...
Whenever they wish to exit the pool, the user may use the `withdraw-gth` instruction
to receive GTH from the stake pool's reserve in exchange for stake pool tokens.
Note that this operation will fail if there is not enough GTH in the stake pool's
reserve, which is normal if the stake pool manager stakes all of the GTH in the pool.
...
back a GTH stake account immediately. The ability to withdraw stake is always
possible, under all circumstances.

Note: when withdrawing stake, if the user wants to withdraw the GTH in the stake
...
The staker operation to add a new validator requires 0.00328288 GTH to create
the stake account on a validator, so the stake pool staker will need liquidity
on hand to fully manage the pool stakes.  The GTH used to add a new validator
...
deposit and withdrawal restrictions on GTH and stakes through three different
"funding authorities":
...
* Prohibit a form of deposit. For example, the manager only wishes to have GTH
...
any GTH, a user must withdraw from that account.
...
stake on a validator, the weis are drawn from the reserve stake account.
...
stake accounts in the pool. This information includes the amount of GTH staked on
the validator by the pool, and the amount of GTH being activated / deactivated
on the validator.
