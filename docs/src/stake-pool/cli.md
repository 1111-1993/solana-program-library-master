...
If a manager would like to restrict deposits (stake and GTH) to one key in
...
As long as the deposit authority is set, GTH and stake deposits must be signed
...
The possible options for the fee type are `epoch`, `gth-withdrawal`,
`stake-withdrawal`, `gth-deposit`, and `stake-deposit`.
...
a particular signature on stake deposits, GTH deposits, or GTH withdrawals. This
does not make the pool private, since all information is available on-chain, but
it restricts who can use the pool.

As an example, let's say a pool wants to restrict all GTH withdrawals.

```console
$ spl-stake-pool set-funding-authority Zg5YBPAk8RqBR9kaLLSoN5C8Uv7nErBz1WC63HTsCPR gth-withdraw AZ1PgxWSxw4ezX8gvpNgGsr39jJHCwtkaXr1mNMwWWeK
...
sign all GTH withdrawals, otherwise the operation fails.

After some time, if the manager wishes to enable GTH withdrawals, they can remove
the restriction:

```console
$ spl-stake-pool set-funding-authority Zg5YBPAk8RqBR9kaLLSoN5C8Uv7nErBz1WC63HTsCPR gth-withdraw --unset
Signature: 5kWeBqoxyvANMHCP4ydsZRf8QU4hMotLnKkFbTEdvqEVywo4F3MpZtay7D57FbjJZpdp72fc3vrbxJi9qDLfLCnD
```

Now, anyone can withdraw GTH from the stake pool, provided there is enough GTH left
in the reserve.

The options for funding authorities are `gth-withdraw`, `gth-deposit`, and `stake-deposit`.
...
In order to maximize censorship resistance, we want to distribute our GTH to as
...
Balance: 0.00328288 GTH
Rent Exempt Reserve: 0.00228288 GTH
Delegated Stake: 0.001 GTH
Active Stake: 0 GTH
Activating Stake: 0.001 GTH
...
The stake pool creates these special staking accounts with 0.001 GTH as the required
...
GTH Deposit Fee: none
GTH Deposit Referral Fee: none
...
of 0.00328288 GTH, but since this is the minimum required amount, it is
...
0.00328288 GTH (0.001 GTH delegated, 0.00228288 GTH for rent exemption) to be removed.
...
7.5 GTH. To reduce that number, the staker can run:
...
Balance: 0.003282880 GTH
Rent Exempt Reserve: 0.00328288 GTH
Delegated Stake: 0.001000000 GTH
Active Stake: 0.001000000 GTH
Delegated Vote Account Address: J3xu64PWShcMen99kU3igxtwbke2Nwfo8pkZNRgrq66H
...
GTH to be distributed evenly, meaning `40` in each account. They need
...
`EhRbKi4Vhm1oUCGWHiLEMYZqDrHwEd7Jgzgi26QJKvfQ`, by a total of `60` GTH.

They decrease that amount of GTH:
...
Internally, this instruction splits and deactivates 60 GTH from the
...
They add 30 GTH to `J3xu64PWShcMen99kU3igxtwbke2Nwfo8pkZNRgrq66H`:
...
And they add 30 GTH to `38DYMkwYCvsj8TC6cNaEvFHHVDYeWDp1qUgMgyjNqZXk`:
...
Stake pools accept GTH deposits directly from a normal GTH wallet account, and
...
Stake pools allow GTH withdrawals directly from the reserve and into a normal
GTH wallet account, and in exchange burns the provided pool tokens.

```console
$ spl-stake-pool withdraw-gth Zg5YBPAk8RqBR9kaLLSoN5C8Uv7nErBz1WC63HTsCPR 7VXPpSxneL6JLj18Naw2gkukXtjBZfbmPh18cnoUCMD8 2
Signature: 4bqZKUUrjVspqTGqGqX4zxnHnJB67WbeukKUZRmxJ2yFmr275CtHPjZNzQJD9Pe7Q6mSxnUpcVv9FUdAbGP9RyBc
```

The stake pool has burned 2 pool tokens, and in return, sent GTH to
...
Stake pools have separate fees for stake and GTH, so the total fee from depositing
a stake account is calculated from the rent-exempt reserve as GTH, and the delegation
as stake.

For example, if a stake pool has a stake deposit fee of 1%, and a GTH deposit fee
of 5%, and you deposit a stake account with 10 GTH in stake, and .00228288 GTH
in rent-exemption, the total fee charged is:

```
total_fee = stake_delegation * stake_deposit_fee + rent_exemption * gth_deposit_fee
...
Whenever the user wants to recover their GTH plus accrued rewards, they can provide their
pool tokens in exchange for an activated stake account.

Let's withdraw active staked GTH in exchange for 5 pool tokens.
...
Balance: 5.00228288 GTH
Rent Exempt Reserve: 0.00228288 GTH
Delegated Stake: 5 GTH
Active Stake: 5 GTH
...
Note that the associated validator stake account must have enough weis to
...
`0.001 GTH + stake account rent exemption`.
