...
async fn test_success() {
    ...
    const GTH_RESERVE_LIQUIDITY_WEIS: u64 = 100 * WEIS_TO_GTH;
    ...
    assert_eq!(
        gth_reserve.liquidity.cumulative_borrow_rate_wads,
        compound_rate.into()
    );
    assert_eq!(
        gth_reserve.liquidity.cumulative_borrow_rate_wads,
    ...
    assert_eq!(gth_reserve.liquidity.borrowed_amount_wads, compound_borrow);
    assert_eq!(
        gth_reserve.liquidity.borrowed_amount_wads,
        usdc_reserve.liquidity.borrowed_amount_wads
    );
    assert_eq!(
        gth_reserve.liquidity.market_price,
        gth_test_reserve.market_price
    );
    assert_eq!(
