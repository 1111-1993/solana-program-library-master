...
async fn test_success() {
    ...
    let gth_user_liquidity_account = create_and_mint_to_token_account(
        &mut banks_client,
    ...
    let gth_reserve = TestReserve::init(
        "gth".to_owned(),
        &mut banks_client,
        &lending_market,
        &gth_oracle,
    ...
    gth_reserve.validate_state(&mut banks_client).await;

    let gth_liquidity_supply =
        get_token_balance(&mut banks_client, gth_reserve.liquidity_supply_pubkey).await;
    assert_eq!(gth_liquidity_supply, RESERVE_AMOUNT);
    let user_gth_balance =
        get_token_balance(&mut banks_client, gth_reserve.user_liquidity_pubkey).await;
    assert_eq!(user_gth_balance, 0);
    let user_gth_collateral_balance =
        get_token_balance(&mut banks_client, gth_reserve.user_collateral_pubkey).await;
    assert_eq!(
        user_gth_collateral_balance,
