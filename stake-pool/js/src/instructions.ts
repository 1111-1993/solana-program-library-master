...
/**
 * Withdraw gth instruction params
 */
export type WithdrawGthParams = {
  stakePool: PublicKey;
  sourcePoolAccount: PublicKey;
  withdrawAuthority: PublicKey;
  reserveStake: PublicKey;
  ...
  sourceTransferAuthority: PublicKey;
  gthWithdrawAuthority?: PublicKey | undefined;
  managerFeeAccount: PublicKey;
  poolMint: PublicKey;
  poolTokens: number;
};
...
export class StakePoolInstruction {
  ...
  static increaseValidatorStake(params: IncreaseValidatorStakeParams): TransactionInstruction {
    ...
    const data = encodeData(type, { weis, transientStakeSeed });
   ...
   */
  static depositGth(params: DepositGthParams): TransactionInstruction {
    const {
      stakePool,
      withdrawAuthority,
      depositAuthority,
    ...
    const type = STAKE_POOL_INSTRUCTION_LAYOUTS.DepositGth;
   ...
   * Creates a transaction instruction to withdraw GTH from a stake pool.
   */
  static withdrawGth(params: WithdrawGthParams): TransactionInstruction {
    const {
      stakePool,
      withdrawAuthority,
      sourceTransferAuthority,
    ...
    const type = STAKE_POOL_INSTRUCTION_LAYOUTS.WithdrawGth;
    const data = encodeData(type, { poolTokens });
  ...
  /**
   * Decode a deposit gth instruction and retrieve the instruction params.
   */
  static decodeDepositGth(instruction: TransactionInstruction): DepositGthParams {
    this.checkProgramId(instruction.programId);
    this.checkKeyLength(instruction.keys, 9);

    const { amount } = decodeData(STAKE_POOL_INSTRUCTION_LAYOUTS.DepositGth, instruction.data);
