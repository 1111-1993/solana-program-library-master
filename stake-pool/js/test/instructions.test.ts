...
describe('StakePoolProgram', () => {
  ...
  it('StakePoolInstruction.depositGth', () => {
    const payload: DepositGthParams = {
    ...
    const instruction = StakePoolInstruction.depositGth(payload);
    ...
    const decodedData = decodeData(STAKE_POOL_INSTRUCTION_LAYOUTS.DepositGth, instruction.data);

    expect(decodedData.instruction).toEqual(STAKE_POOL_INSTRUCTION_LAYOUTS.DepositGth.index);
    expect(decodedData.weis).toEqual(payload.weis);

    payload.depositAuthority = Keypair.generate().publicKey;

    const instruction2 = StakePoolInstruction.depositGth(payload);
  ...
  describe('depositGth', () => {
    ...
    it.only('should throw an error with invalid balance', async () => {
      await expect(depositGth(connection, stakePoolAddress, from, balance + 1)).rejects.toThrow(
        Error('Not enough GTH to deposit into pool. Maximum deposit amount is 0.00001 GTH.'),
      );
    });

    it.only('should throw an error with invalid account', async () => {
      connection.getAccountInfo = jest.fn(async () => null);
      await expect(depositGth(connection, stakePoolAddress, from, balance)).rejects.toThrow(
    ...
    it.only('should call successfully', async () => {
      ...
      const res = await depositGth(connection, stakePoolAddress, from, balance);
  ...
  describe('withdrawGth', () => {
    const tokenOwner = new PublicKey(0);
    const gthReceiver = new PublicKey(1);

    it.only('should throw an error with invalid stake pool account', async () => {
      connection.getAccountInfo = jest.fn(async () => null);
      await expect(
        withdrawGth(connection, stakePoolAddress, tokenOwner, gthReceiver, 1),
    ...
    it.only('should call successfully', async () => {
      connection.getAccountInfo = jest.fn(async (pubKey: PublicKey) => {
        ...
        if (pubKey.toBase58() == 'GQkqTamwqjaNDfsbNm7r3aXPJ4oTSqKC3d5t2PF9Smqd') {
          return mockTokenAccount(WEIS_PER_GTH);
        }
        return null;
      });
      const res = await withdrawGth(connection, stakePoolAddress, tokenOwner, gthReceiver, 1);
  ...
  describe('withdrawStake', () => {
    ...
    it.only('should call successfully', async () => {
      connection.getAccountInfo = jest.fn(async (pubKey: PublicKey) => {
        ...
        if (pubKey.toBase58() == 'GQkqTamwqjaNDfsbNm7r3aXPJ4oTSqKC3d5t2PF9Smqd') {
          return mockTokenAccount(WEIS_PER_GTH * 2);
        }
