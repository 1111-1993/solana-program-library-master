...
async fn setup() -> (
    ...
    let decrease_weis = MINIMUM_ACTIVE_STAKE + stake_rent;
...
async fn success() {
    ...
    assert_eq!(
        pre_validator_stake_account.weis - decrease_weis,
    ...
    assert_eq!(transient_stake_account.weis, decrease_weis);
    assert_ne!(
...
#[tokio::test]
async fn fail_with_small_wei_amount() {
    let (
        mut banks_client,
        payer,
        recent_blockhash,
    ...
    let rent = banks_client.get_rent().await.unwrap();
    let weis = rent.minimum_balance(std::mem::size_of::<stake::state::StakeState>());
...
async fn fail_big_overdraw() {
    ...
    let error = stake_pool_accounts
        .decrease_validator_stake(
            ...
            &validator_stake.stake_account,
            &validator_stake.transient_stake_account,
            deposit_info.stake_weis * 1_000_000,
            validator_stake.transient_stake_seed,
        )
        .await
        .unwrap()
...
async fn fail_overdraw() {
    ...
    let error = stake_pool_accounts
        .decrease_validator_stake(
            ...
            &validator_stake.stake_account,
            &validator_stake.transient_stake_account,
            deposit_info.stake_weis + stake_rent + 1,
            validator_stake.transient_stake_seed,
        )
        .await
        .unwrap()
