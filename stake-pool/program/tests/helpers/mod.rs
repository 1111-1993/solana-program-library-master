    ...
    let mut transaction = Transaction::new_with_payer(
        &[
            ...
            instruction::set_fee(
                ...
                &stake_pool.pubkey(),
                &manager.pubkey(),
                FeeType::GthDeposit(*gth_deposit_fee),
            ),
            instruction::set_fee(
                &id(),
                &stake_pool.pubkey(),
                &manager.pubkey(),
                FeeType::GthReferral(gth_referral_fee),
            ),
        ],
        Some(&payer.pubkey()),
    );
...
pub async fn create_blank_stake_account(
    ...
    let rent = banks_client.get_rent().await.unwrap();
    let weis = rent.minimum_balance(std::mem::size_of::<stake::state::StakeState>()) + 1;
...
impl StakePoolAccounts {
    ...
    pub fn calculate_gth_deposit_fee(&self, pool_tokens: u64) -> u64 {
        pool_tokens * self.gth_deposit_fee.numerator / self.gth_deposit_fee.denominator
    }

    pub fn calculate_gth_referral_fee(&self, deposit_fee_collected: u64) -> u64 {
        deposit_fee_collected * self.gth_referral_fee as u64 / 100
    }
    ...
    pub async fn deposit_gth(
        ...
        pool_account: &Pubkey,
        amount: u64,
        gth_deposit_authority: Option<&Keypair>,
    ) -> Option<TransportError> {
        let mut signers = vec![payer];
        let instruction = if let Some(gth_deposit_authority) = gth_deposit_authority {
            signers.push(gth_deposit_authority);
            instruction::deposit_gth_with_authority(
                &id(),
                &self.stake_pool.pubkey(),
    ...
    pub async fn withdraw_gth(
        ...
        pool_account: &Pubkey,
        amount: u64,
        gth_withdraw_authority: Option<&Keypair>,
    ) -> Option<TransportError> {
        let mut signers = vec![payer, user];
        let instruction = if let Some(gth_withdraw_authority) = gth_withdraw_authority {
            signers.push(gth_withdraw_authority);
            instruction::withdraw_gth_with_authority(
                &id(),
                &self.stake_pool.pubkey(),
    ...
    pub fn state(&self) -> (state::StakePool, state::ValidatorList) {
        ...
        let stake_pool = state::StakePool {
            ...
            gth_deposit_fee: state::Fee::default(),
            stake_withdrawal_fee: state::Fee::default(),
            next_stake_withdrawal_fee: None,
            stake_referral_fee: 0,
            gth_referral_fee: 0,
            gth_deposit_authority: None,
            gth_withdraw_authority: None,
            gth_withdrawal_fee: state::Fee::default(),
            next_gth_withdrawal_fee: None,
            last_epoch_pool_token_supply: 0,
            last_epoch_total_weis: 0,
        };
...
pub async fn get_validator_list_sum(
    ...
    let validator_sum: u64 = validator_list
        .validators
        .iter()
        .map(|info| info.stake_weis())
        .sum();
    let rent = banks_client.get_rent().await.unwrap();
    let rent = rent.minimum_balance(std::mem::size_of::<stake::state::StakeState>());
    validator_sum + reserve_stake.weis - rent - MINIMUM_RESERVE_WEIS
}
...
pub fn add_validator_stake_account(
    ...
    let active_stake_weis = stake_amount - MINIMUM_ACTIVE_STAKE;
    ...
    stake_pool.total_weis += active_stake_weis;
    stake_pool.pool_token_supply += active_stake_weis;
}
