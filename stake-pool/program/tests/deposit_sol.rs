...
async fn success() {
    ...
    assert_eq!(
        ...
        pre_stake_pool.total_weis + TEST_STAKE_AMOUNT
    );
    assert_eq!(
    ...
    let tokens_issued_user =
        tokens_issued - stake_pool_accounts.calculate_gth_deposit_fee(tokens_issued);
    ...
    assert_eq!(
        post_reserve_weis,
        pre_reserve_weis + TEST_STAKE_AMOUNT
    );
}
...
#[tokio::test]
async fn success_with_gth_deposit_authority() {
    ...
    let gth_deposit_authority = Keypair::new();
    ...
    let mut transaction = Transaction::new_with_payer(
        &[instruction::set_funding_authority(
            ...
            Some(&gth_deposit_authority.pubkey()),
            FundingType::GthDeposit,
        )],
        Some(&payer.pubkey()),
    );
...
#[tokio::test]
async fn fail_without_gth_deposit_authority_signature() {
    let (mut banks_client, payer, recent_blockhash) = program_test().start().await;
    let gth_deposit_authority = Keypair::new();
    let stake_pool_accounts = StakePoolAccounts::new();
    stake_pool_accounts
    ...
    match error {
        ...
        _ => panic!("Wrong error occurs while trying to make a deposit without GTH deposit authority signature"),
    }
}
...
async fn success_with_referral_fee() {
    ...
    let referrer_balance_post =
        get_token_balance(&mut context.banks_client, &referrer_token_account.pubkey()).await;
    let referral_fee = stake_pool_accounts.calculate_gth_referral_fee(
        stake_pool_accounts.calculate_gth_deposit_fee(TEST_STAKE_AMOUNT),
    );
