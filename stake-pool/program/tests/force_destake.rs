...
async fn setup() -> (ProgramTestContext, StakePoolAccounts, Pubkey) {
    ...
    let active_stake_weis = TEST_STAKE_AMOUNT - MINIMUM_ACTIVE_STAKE;
...
async fn success_update() {
    ...
    let validator_stake_weis = context
        .banks_client
    ...
    assert_eq!(
        post_reserve_weis,
        pre_reserve_weis + validator_stake_weis
    );
