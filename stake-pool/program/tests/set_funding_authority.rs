...
#[tokio::test]
async fn success_set_gth_deposit_authority() {
    let (mut banks_client, payer, recent_blockhash, stake_pool_accounts, new_gth_deposit_authority) =
        setup().await;
    ...
    let mut transaction = Transaction::new_with_payer(
        &[instruction::set_funding_authority(
            ...
            &stake_pool_accounts.stake_pool.pubkey(),
            &stake_pool_accounts.manager.pubkey(),
            Some(&new_gth_deposit_authority.pubkey()),
    ...
    assert_eq!(
        stake_pool.gth_deposit_authority,
        Some(new_gth_deposit_authority.pubkey())
    );
    ...
    assert_eq!(stake_pool.gth_deposit_authority, None);
}
...
async fn success_set_withdraw_authority() {
    ...
    assert_eq!(
        stake_pool.gth_withdraw_authority,
        Some(new_authority.pubkey())
    );
    ...
    assert_eq!(stake_pool.gth_withdraw_authority, None);
}
