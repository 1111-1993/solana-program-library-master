...
async fn setup() -> (
    ...
    let stake_weis = create_independent_stake_account(
        &mut context.banks_client,
        &context.payer,
        &context.last_blockhash,
        &deposit_stake,
...
async fn success() {
    ...
    let pre_reserve_weis = get_account(
        &mut context.banks_client,
    ...
    let tokens_issued = stake_weis; // For now tokens are 1:1 to stake
    ...
    assert_eq!(
        post_stake_pool.total_weis,
        pre_stake_pool.total_weis + stake_weis
    );
    assert_eq!(
    ...
    let tokens_issued_user = tokens_issued
        - post_stake_pool
            .calc_pool_tokens_gth_deposit_fee(stake_rent)
            .unwrap()
        - post_stake_pool
            .calc_pool_tokens_stake_deposit_fee(stake_weis - stake_rent)
            .unwrap();
    ...
    let post_validator_stake_item = validator_list
        ...
        .unwrap();
    assert_eq!(
        post_validator_stake_item.stake_weis(),
        pre_validator_stake_item.stake_weis() + stake_weis - stake_rent,
    );

    // Check validator stake account actual GTH balance
    ...
    assert_eq!(post_validator_stake_item.transient_stake_weis, 0);

    // Check reserve
    let post_reserve_weis = get_account(
        &mut context.banks_client,
        &stake_pool_accounts.reserve_stake.pubkey(),
    )
    .await
    .weis;
    assert_eq!(post_reserve_weis, pre_reserve_weis + stake_rent);
}

#[tokio::test]
async fn success_with_extra_stake_weis() {
    let (
        mut context,
        stake_pool_accounts,
        validator_stake_account,
    ...
    let extra_weis = TEST_STAKE_AMOUNT * 3 + 1;
    ...
    let tokens_issued = stake_weis + extra_weis;
    // For now tokens are 1:1 to stake

    // Stake pool should add its balance to the pool balance

    // The extra weis will not get recorded in total stake weis unless
    ...
    assert_eq!(
        post_stake_pool.total_weis,
        pre_stake_pool.total_weis + extra_weis + stake_weis
    );
    assert_eq!(
        post_stake_pool.pool_token_supply,
        pre_stake_pool.pool_token_supply + tokens_issued
    ...
    let fee_tokens = post_stake_pool
        .calc_pool_tokens_gth_deposit_fee(extra_weis + stake_rent)
        .unwrap()
        + post_stake_pool
            .calc_pool_tokens_stake_deposit_fee(stake_weis - stake_rent)
            .unwrap();
    ...
    assert_eq!(
        post_reserve_weis,
        pre_reserve_weis + stake_rent + extra_weis
    );
}
