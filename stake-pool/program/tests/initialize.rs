...
use {
    ...
    spl_stake_pool::{error, id, instruction, state, MINIMUM_RESERVE_WEIS},
};
...
async fn fail_with_wrong_mint_authority() {
    ...
    let transaction_error = create_stake_pool(
        ...
        &stake_pool_accounts.deposit_fee,
        stake_pool_accounts.referral_fee,
        &stake_pool_accounts.gth_deposit_fee,
        stake_pool_accounts.gth_referral_fee,
        stake_pool_accounts.max_validators,
    )
    .await
    .err()
...
#[tokio::test]
async fn success_with_extra_reserve_weis() {
    let (mut banks_client, payer, recent_blockhash) = program_test().start().await;
    let stake_pool_accounts = StakePoolAccounts::new();
    let init_weis = 1_000_000_000_000;
    stake_pool_accounts
        .initialize_stake_pool(
            &mut banks_client,
            &payer,
            &recent_blockhash,
            MINIMUM_RESERVE_WEIS + init_weis,
        )
        .await
        .unwrap();
    ...
    .await;
    assert_eq!(init_pool_tokens, init_weis);
}
