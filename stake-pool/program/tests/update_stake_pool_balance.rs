...
use {
    ...
    spl_stake_pool::{error::StakePoolError, state::StakePool, MINIMUM_RESERVE_WEIS},
};
...
async fn success() {
    ...
    assert_eq!(pre_balance, stake_pool.total_weis);
    ...
    .await;
    let stake_pool = try_from_slice_unchecked::<StakePool>(stake_pool.data.as_slice()).unwrap();
    assert_eq!(post_balance, stake_pool.total_weis);
    ...
    let expected_fee_weis = (post_balance - pre_balance) * stake_pool.epoch_fee.numerator
        / stake_pool.epoch_fee.denominator;
    let actual_fee_weis = stake_pool.calc_pool_tokens_for_deposit(actual_fee).unwrap();
    assert_eq!(actual_fee_weis, expected_fee_weis);

    let expected_fee = expected_fee_weis * pool_token_supply / post_balance;
    ...
    assert_eq!(pre_balance, stake_pool.last_epoch_total_weis);
}

#[tokio::test]
async fn success_ignoring_extra_weis() {
    let (mut context, stake_pool_accounts, stake_accounts) = setup().await;
