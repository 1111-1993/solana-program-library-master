...
async fn success() {
    ...
    assert_eq!(
        ...
        pre_stake_pool.total_weis - amount_withdrawn_minus_fee
    );
    assert_eq!(
    ...
    assert_eq!(
        post_reserve_weis,
        pre_reserve_weis - amount_withdrawn_minus_fee
    );
}
...
async fn fail_overdraw_reserve() {
    ...
    // try to withdraw one wei, will overdraw
    let error = stake_pool_accounts
        .withdraw_gth(
            &mut context.banks_client,
            &context.payer,
...
#[tokio::test]
async fn success_with_gth_withdraw_authority() {
    let (mut context, stake_pool_accounts, user, pool_token_account, pool_tokens) = setup().await;
    let gth_withdraw_authority = Keypair::new();
    ...
    let transaction = Transaction::new_signed_with_payer(
        &[instruction::set_funding_authority(
            ...
            Some(&gth_withdraw_authority.pubkey()),
            FundingType::GthWithdraw,
        )],
...
#[tokio::test]
async fn fail_without_gth_withdraw_authority_signature() {
    let (mut context, stake_pool_accounts, user, pool_token_account, pool_tokens) = setup().await;
    let gth_withdraw_authority = Keypair::new();
