...
use {
    ...
    spl_stake_pool::{
        ...
        MAX_VALIDATORS_TO_UPDATE, MINIMUM_ACTIVE_STAKE, MINIMUM_RESERVE_WEIS,
    },
...
async fn setup(
    ...
    stake_pool_accounts
        .initialize_stake_pool(
            ...
            reserve_stake_amount + MINIMUM_RESERVE_WEIS,
        )
        .await
        .unwrap();
...
async fn success() {
    ...
    let initial_weis =
        (validator_weis + stake_rent) * num_validators as u64 + reserve_weis;
    assert_eq!(
    ...
    stake_pool_accounts
        ...
        .await;
    let new_weis = get_validator_list_sum(
        &mut context.banks_client,
        &stake_pool_accounts.reserve_stake.pubkey(),
        &stake_pool_accounts.validator_list.pubkey(),
    )
    .await;
    assert!(new_weis > initial_weis);
    ...
    assert_eq!(new_weis, stake_pool.total_weis);
}

#[tokio::test]
async fn merge_into_reserve() {
    let (mut context, stake_pool_accounts, stake_accounts, _, weis, _, mut slot) =
        setup(MAX_VALIDATORS_TO_UPDATE).await;

    let pre_weis = get_validator_list_sum(
        &mut context.banks_client,
        &stake_pool_accounts.reserve_stake.pubkey(),
        &stake_pool_accounts.validator_list.pubkey(),
    )
    ...
    let reserve_stake = context
        ...
        .unwrap()
        .unwrap();
    let pre_reserve_weis = reserve_stake.weis;
    ...
    let expected_weis = get_validator_list_sum(
        &mut context.banks_client,
        &stake_pool_accounts.reserve_stake.pubkey(),
        &stake_pool_accounts.validator_list.pubkey(),
    )
    .await;
    assert_eq!(pre_weis, expected_weis);
    ...
    .await;
    let stake_pool = try_from_slice_unchecked::<StakePool>(&stake_pool_info.data).unwrap();
    assert_eq!(expected_weis, stake_pool.total_weis);
    ...
    let reserve_stake = context
        ...
        .unwrap()
        .unwrap();
    let post_reserve_weis = reserve_stake.weis;
    assert!(post_reserve_weis > pre_reserve_weis);
...
async fn merge_into_validator_stake() {
    let (mut context, stake_pool_accounts, stake_accounts, _, weis, reserve_weis, mut slot) =
        setup(MAX_VALIDATORS_TO_UPDATE).await;
    ...
    for stake_account in &stake_accounts {
        let error = stake_pool_accounts
            .increase_validator_stake(
                ...
                &stake_account.stake_account,
                &stake_account.vote.pubkey(),
                reserve_weis / stake_accounts.len() as u64,
                stake_account.transient_stake_seed,
            )
            .await;
        assert!(error.is_none());
    ...
    let error = stake_pool_accounts
        ...
        .await;
    assert!(error.is_none());
    let current_weis = get_validator_list_sum(
        &mut context.banks_client,
        &stake_pool_accounts.reserve_stake.pubkey(),
        &stake_pool_accounts.validator_list.pubkey(),
    )
    ...
    .await;
    let stake_pool = try_from_slice_unchecked::<StakePool>(&stake_pool_info.data).unwrap();
    assert_eq!(current_weis, stake_pool.total_weis);
    ...
    // validator stake account minimum + deposited weis + rents + increased weis
    let stake_rent = rent.minimum_balance(std::mem::size_of::<StakeState>());
    let expected_weis = MINIMUM_ACTIVE_STAKE
        + weis
        + reserve_weis / stake_accounts.len() as u64
        + stake_rent;
    for stake_account in &stake_accounts {
        ...
        assert_eq!(validator_stake.weis, expected_weis);
    }

    // Check reserve stake accounts for expected balance:
    // own rent, other account rents, and 1 extra wei
    ...
    assert_eq!(
        reserve_stake.weis,
        MINIMUM_RESERVE_WEIS + stake_rent * (1 + stake_accounts.len() as u64)
    );
}
...
async fn merge_transient_stake_after_remove() {
    ...
    assert_eq!(validator_list.validators[0].active_stake_weis, 0);
    assert_eq!(
        validator_list.validators[0].transient_stake_weis,
        deactivated_weis
    );

    // Update with merge, status should be ReadyForRemoval and no weis
    let error = stake_pool_accounts
    ...
    assert_eq!(validator_list.validators[0].stake_weis(), 0);
    ...
    assert_eq!(
        reserve_stake.weis,
        reserve_weis + deactivated_weis + 2 * stake_rent + MINIMUM_RESERVE_WEIS
    );
...
async fn check_ignored_hijacked_transient_stake(
    ...
    let transaction = Transaction::new_signed_with_payer(
        &[
            ...
            system_instruction::transfer(
                ...
                stake_rent + MINIMUM_RESERVE_WEIS,
            ),
    ...
    println!("Update again normally, should be no change in the weis");
    stake_pool_accounts
        .update_all(
            &mut context.banks_client,
            &context.payer,
    ...
    .await;
    let stake_pool = try_from_slice_unchecked::<StakePool>(&stake_pool_info.data).unwrap();
    assert_eq!(pre_weis, stake_pool.total_weis);
}
