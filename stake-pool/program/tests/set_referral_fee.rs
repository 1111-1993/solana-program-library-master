...
async fn success_gth() {
    ...
    let transaction = Transaction::new_signed_with_payer(
        &[instruction::set_fee(
            ...
            &stake_pool_accounts.stake_pool.pubkey(),
            &stake_pool_accounts.manager.pubkey(),
            FeeType::GthReferral(new_referral_fee),
        )],
        Some(&context.payer.pubkey()),
        &[&context.payer, &stake_pool_accounts.manager],
        context.last_blockhash,
    ...
    .await;
    let stake_pool = try_from_slice_unchecked::<StakePool>(stake_pool.data.as_slice()).unwrap();
    assert_eq!(stake_pool.gth_referral_fee, new_referral_fee);
}

#[tokio::test]
async fn fail_gth_wrong_manager() {
    let (mut context, stake_pool_accounts, new_referral_fee) = setup(None).await;
...
#[tokio::test]
async fn fail_gth_high_referral_fee() {
    let (mut context, stake_pool_accounts, _new_referral_fee) = setup(None).await;
