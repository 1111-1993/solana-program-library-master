...
use {
    ...
    spl_stake_pool::{
        error::StakePoolError, id, instruction, minimum_stake_weis, state,
...
async fn _success(test_type: SuccessTestType) {
    ...
    let initial_stake_weis = get_account(&mut banks_client, &user_stake_recipient.pubkey())
        .await
        .weis;
    ...
    // first and only deposit, weis:pool 1:1
    ...
    assert_eq!(
        stake_pool.total_weis,
        stake_pool_before.total_weis - tokens_burnt
    );
    assert_eq!(
    ...
    let validator_stake_item = validator_list
        ...
        .unwrap();
    assert_eq!(
        validator_stake_item.stake_weis(),
        validator_stake_item_before.stake_weis() - tokens_burnt
    );
    assert_eq!(
        validator_stake_item.active_stake_weis,
        validator_stake_item.stake_weis(),
    );
    ...
    assert_eq!(
        validator_stake_account.weis - minimum_stake_weis(&meta),
        validator_stake_item.active_stake_weis
    );
    ...
    assert_eq!(
        user_stake_recipient_account.weis,
        initial_stake_weis + tokens_burnt
    );
}
...
async fn fail_overdraw_validator() {
    ...
    assert_eq!(
        ...
        TransactionError::InstructionError(
            0,
            InstructionError::Custom(StakePoolError::StakeWeisNotEqualToMinimum as u32)
        ),
    );
}
...
async fn success_with_reserve() {
    ...
    let stake_pool_accounts = StakePoolAccounts::new();
    let initial_reserve_weis = MINIMUM_RESERVE_WEIS;
    stake_pool_accounts
        .initialize_stake_pool(
            &mut context.banks_client,
            &context.payer,
            &context.last_blockhash,
            initial_reserve_weis,
        )
        .await
        .unwrap();
    ...
    let deposit_weis = (MINIMUM_ACTIVE_STAKE + stake_rent) * 2;
    ...
    let initial_stake_weis = create_blank_stake_account(
        &mut context.banks_client,
        &context.payer,
        &context.last_blockhash,
    ...
    let deposit_fee = stake_pool
        .calc_pool_tokens_stake_deposit_fee(stake_rent + deposit_info.stake_weis)
        .unwrap();
    assert_eq!(
        deposit_info.stake_weis + stake_rent - deposit_fee,
        deposit_info.pool_tokens,
        "stake {} rent {} deposit fee {} pool tokens {}",
        deposit_info.stake_weis,
        stake_rent,
        deposit_fee,
        deposit_info.pool_tokens
    );
    ...
    let meta = stake_state.meta().unwrap();
    assert_eq!(
        initial_reserve_weis + meta.rent_exempt_reserve + withdrawal_fee + deposit_fee,
        reserve_stake_account.weis
    );
    ...
    assert_eq!(
        user_stake_recipient_account.weis,
        initial_stake_weis + deposit_info.stake_weis + stake_rent
            - withdrawal_fee
            - deposit_fee
    );
}
...
async fn fail_withdraw_from_transient() {
    ...
    // decrease to minimum stake + 1 wei
    let error = stake_pool_accounts
        .decrease_validator_stake(
            &mut context.banks_client,
            &context.payer,
    ...
    let withdraw_destination = Keypair::new();
    let withdraw_destination_authority = Pubkey::new_unique();
    let _initial_stake_weis = create_blank_stake_account(
        &mut context.banks_client,
        &context.payer,
        &context.last_blockhash,
        &withdraw_destination,
    ...
    // fail withdrawing from transient, still a wei in the validator stake account
    let error = stake_pool_accounts
        .withdraw_stake(
            &mut context.banks_client,
            &context.payer,
...
async fn success_withdraw_from_transient() {
    ...
    let deposit_weis = (MINIMUM_ACTIVE_STAKE + stake_rent) * 3;
...
async fn success_empty_out_stake_with_fee() {
    ...
    let stake_state =
        deserialize::<stake::state::StakeState>(&validator_stake_account.data).unwrap();
    let meta = stake_state.meta().unwrap();
    let weis_to_withdraw = validator_stake_account.weis - minimum_stake_weis(&meta);
    ...
    let pool_tokens_to_withdraw =
        weis_to_withdraw * inverse_fee.denominator / inverse_fee.numerator;
    ...
    let meta = stake_state.meta().unwrap();
    assert_eq!(
        validator_stake_account.weis,
        minimum_stake_weis(&meta)
    );
}
