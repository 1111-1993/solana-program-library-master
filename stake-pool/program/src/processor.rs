...
use {
    crate::{
        ...
        minimum_reserve_weis, minimum_stake_weis,
        state::{
...
/// Create a transient stake account without transferring weis
...
fn create_pda_account<'a>(
    ...
    if new_pda_account.weis() > 0 {
        let required_weis = required_weis.saturating_sub(new_pda_account.weis());
        if required_weis > 0 {
            invoke(
                &system_instruction::transfer(payer.key, new_pda_account.key, required_weis),
                &[
                    payer.clone(),
...
impl Processor {
    ...
    /// Issue stake::instruction::withdraw instruction to move additional weis
    #[allow(clippy::too_many_arguments)]
    fn stake_withdraw<'a>(
        stake_pool: &Pubkey,
        source_account: AccountInfo<'a>,
    ...
    fn process_initialize(
        ...
        if !rent.is_exempt(stake_pool_info.weis(), stake_pool_info.data_len()) {
            msg!("Stake pool not rent-exempt");
            return Err(ProgramError::AccountNotRentExempt);
        }

        if !rent.is_exempt(
            validator_list_info.weis(),
        ...
        let (stake_deposit_authority, gth_deposit_authority) =
        ...
        let total_weis = if let stake::state::StakeState::Initialized(meta) = stake_state {
            ...
            reserve_stake_info
                .weis()
                .checked_sub(minimum_reserve_weis(&meta))
        ...
        stake_pool.total_weis = total_weis;
        stake_pool.pool_token_supply = total_weis;
        ...
        stake_pool.gth_deposit_authority = gth_deposit_authority;
        stake_pool.gth_deposit_fee = deposit_fee;
        stake_pool.gth_referral_fee = referral_fee;
        stake_pool.gth_withdraw_authority = None;
        stake_pool.gth_withdrawal_fee = withdrawal_fee;
        stake_pool.next_gth_withdrawal_fee = None;
        stake_pool.last_epoch_pool_token_supply = 0;
        stake_pool.last_epoch_total_weis = 0;
    ...
    fn process_add_validator_to_pool(
        ...
        let required_weis = MINIMUM_ACTIVE_STAKE + rent.minimum_balance(space);
    ...
    fn process_remove_validator_from_pool(
        ...
        let stake_weis = **stake_account_info.weis.borrow();
        let required_weis = minimum_stake_weis(&meta);
        if stake_weis != required_weis {
            msg!(
                "Attempting to remove validator account with {} weis, must have {} weis",
                stake_weis,
                required_weis
            );
            return Err(StakePoolError::StakeWeisNotEqualToMinimum.into());
        }

        if stake.delegation.stake != MINIMUM_ACTIVE_STAKE {
            msg!(
                "Error: attempting to remove stake with delegation of {} weis, must have {} weis",
        ...
        let new_status = if validator_stake_info.transient_stake_weis > 0 {
    ...
    fn process_decrease_validator_stake(
        ...
        let mut validator_stake_info = maybe_validator_stake_info.unwrap();
        if validator_stake_info.transient_stake_weis > 0 {
        ...
        if weis <= stake_rent {
            msg!(
                "Need more than {} weis for transient stake to be rent-exempt, {} provided",
                stake_rent,
                weis
            );
            return Err(ProgramError::AccountNotRentExempt);
        ...
        let remaining_weis = validator_stake_account_info
            .weis()
            .checked_sub(weis)
            .ok_or(ProgramError::InsufficientFunds)?;
        let required_weis = minimum_stake_weis(&meta);
        if remaining_weis < required_weis {
            msg!("Need at least {} weis in the stake account after decrease, {} requested, {} is the current possible maximum",
                required_weis,
                weis,
                validator_stake_account_info.weis().checked_sub(required_weis).ok_or(StakePoolError::CalculationFailure)?
            );
        ...
        validator_stake_info.active_stake_weis = validator_stake_info
            .active_stake_weis
            .checked_sub(weis)
            .ok_or(StakePoolError::CalculationFailure)?;
        validator_stake_info.transient_stake_weis = weis;
    ...
    fn process_increase_validator_stake(
        ...
        let stake_rent = rent.minimum_balance(std::mem::size_of::<stake::state::StakeState>());
        if weis < MINIMUM_ACTIVE_STAKE {
            msg!(
                "Need more than {} weis for transient stake to be rent-exempt and mergeable, {} provided",
                MINIMUM_ACTIVE_STAKE,
                weis
            );
            return Err(ProgramError::AccountNotRentExempt);
        ...
        let total_weis = weis.saturating_add(stake_rent);

        if reserve_stake_account_info
            .weis()
            .saturating_sub(total_weis)
            <= stake_rent
        {
            ...
            msg!(
                "Reserve stake does not have enough weis for increase, must be less than {}, {} requested",
                max_split_amount,
                weis
            );
            return Err(ProgramError::InsufficientFunds);
        ...
        validator_stake_info.transient_stake_weis = total_weis;
        validator_stake_info.transient_seed_suffix_start = transient_stake_seed;
    ...
    fn process_update_validator_list_balance(
        ...
        for (validator_stake_record, validator_stakes) in validator_iter {
            ...
            let mut active_stake_weis = 0;
            let mut transient_stake_weis = 0;
            ...
            //  * activating -> nothing, just account its weis
            //  * deactivating -> nothing, just account its weis
            ...
            match transient_stake_state {
                Some(stake::state::StakeState::Initialized(meta)) => {
                    if stake_is_usable_by_pool(
                        ...
                        if no_merge {
                            transient_stake_weis = transient_stake_info.weis();
                        } else {
                            // merge into reserve
                            Self::stake_merge(
                                stake_pool_info.key,
                ...
                Some(stake::state::StakeState::Stake(meta, stake)) => {
                    if stake_is_usable_by_pool(
                        ...
                        if no_merge {
                            transient_stake_weis = account_stake;
                                ...
                                validator_stake_state
                            {
                                if validator_stake.delegation.activation_epoch < clock.epoch {
                                    let additional_weis = transient_stake_info
                                        .weis()
            ...
            match validator_stake_state {
                    ...
                    ) =>
                {
                    ...
                    // absorb those weis into the reserve.  The transient
            ...
            validator_stake_record.active_stake_weis = active_stake_weis;
            validator_stake_record.transient_stake_weis = transient_stake_weis;
        }
    ...
    fn process_update_stake_pool_balance(
        ...
        let previous_weis = stake_pool.total_weis;
        ...
        let mut total_weis = if let stake::state::StakeState::Initialized(meta) = reserve_stake
        {
            reserve_stake_info
                .weis()
                .checked_sub(minimum_reserve_weis(&meta))
        ...
        for validator_stake_record in validator_list.iter::<ValidatorStakeInfo>() {
            ...
            total_weis = total_weis
                .checked_add(validator_stake_record.stake_weis())
                .ok_or(StakePoolError::CalculationFailure)?;
        }

        let reward_weis = total_weis.saturating_sub(previous_weis);
        ...
        let fee = if stake_pool.check_manager_fee_info(manager_fee_info).is_ok() {
            stake_pool
                .calc_epoch_fee_amount(reward_weis)
                .ok_or(StakePoolError::CalculationFailure)?
        } else {
            0
        };
        ...
        if stake_pool.last_update_epoch < clock.epoch {
            ...
            if let Some(fee) = stake_pool.next_gth_withdrawal_fee {
                stake_pool.gth_withdrawal_fee = fee;
                stake_pool.next_gth_withdrawal_fee = None;
            }
            stake_pool.last_update_epoch = clock.epoch;
            stake_pool.last_epoch_total_weis = previous_weis;
    ...
    fn process_deposit_stake(program_id: &Pubkey, accounts: &[AccountInfo]) -> ProgramResult {
        ...
        let pre_all_validator_weis = validator_stake_account_info.weis();
        ...
        let post_all_validator_weis = validator_stake_account_info.weis();
        msg!("Stake post merge {}", post_validator_stake.delegation.stake);

        let total_deposit_weis = post_all_validator_weis
            .checked_sub(pre_all_validator_weis)
            .ok_or(StakePoolError::CalculationFailure)?;
        let stake_deposit_weis = post_validator_stake
            .delegation
            .stake
            .checked_sub(validator_stake.delegation.stake)
            .ok_or(StakePoolError::CalculationFailure)?;
        let gth_deposit_weis = total_deposit_weis
            .checked_sub(stake_deposit_weis)
            .ok_or(StakePoolError::CalculationFailure)?;

        let new_pool_tokens = stake_pool
            .calc_pool_tokens_for_deposit(total_deposit_weis)
            .ok_or(StakePoolError::CalculationFailure)?;
        let new_pool_tokens_from_stake = stake_pool
            .calc_pool_tokens_for_deposit(stake_deposit_weis)
            .ok_or(StakePoolError::CalculationFailure)?;
        let new_pool_tokens_from_gth = new_pool_tokens
        ...
        let stake_deposit_fee = stake_pool
            ...
            .ok_or(StakePoolError::CalculationFailure)?;
        let gth_deposit_fee = stake_pool
            .calc_pool_tokens_gth_deposit_fee(new_pool_tokens_from_gth)
            .ok_or(StakePoolError::CalculationFailure)?;

        let total_fee = stake_deposit_fee
            .checked_add(gth_deposit_fee)
            .ok_or(StakePoolError::CalculationFailure)?;
        ...
        // withdraw additional weis to the reserve
        if gth_deposit_weis > 0 {
            Self::stake_withdraw(
                stake_pool_info.key,
                validator_stake_account_info.clone(),
                withdraw_authority_info.clone(),
        ...
        stake_pool.pool_token_supply = stake_pool
            ...
            .ok_or(StakePoolError::CalculationFailure)?;
        // We treat the extra weis as though they were
        // transferred directly to the reserve stake account.
        stake_pool.total_weis = stake_pool
            .total_weis
            .checked_add(total_deposit_weis)
            .ok_or(StakePoolError::CalculationFailure)?;
        stake_pool.serialize(&mut *stake_pool_info.data.borrow_mut())?;

        validator_stake_info.active_stake_weis = post_validator_stake
            .delegation
            .stake
            .checked_sub(MINIMUM_ACTIVE_STAKE)
            .ok_or(StakePoolError::CalculationFailure)?;
    ...
    /// Processes [DepositGth](enum.Instruction.html).
    #[inline(never)] // needed to avoid stack size violation
    fn process_deposit_gth(
        program_id: &Pubkey,
        accounts: &[AccountInfo],
        deposit_weis: u64,
    ) -> ProgramResult {
        ...
        let withdraw_authority_info = next_account_info(account_info_iter)?;
        let reserve_stake_account_info = next_account_info(account_info_iter)?;
        let from_user_weis_info = next_account_info(account_info_iter)?;
        let dest_user_pool_info = next_account_info(account_info_iter)?;
        let manager_fee_info = next_account_info(account_info_iter)?;
        let referrer_fee_info = next_account_info(account_info_iter)?;
        let pool_mint_info = next_account_info(account_info_iter)?;
        let system_program_info = next_account_info(account_info_iter)?;
        let token_program_info = next_account_info(account_info_iter)?;
        let gth_deposit_authority_info = next_account_info(account_info_iter);
        ...
        stake_pool.check_gth_deposit_authority(gth_deposit_authority_info)?;
        stake_pool.check_mint(pool_mint_info)?;
        stake_pool.check_reserve_stake(reserve_stake_account_info)?;
        ...
        // We want this to hold to ensure that deposit_gth mints pool tokens
        // at the right price
        if stake_pool.last_update_epoch < clock.epoch {
            return Err(StakePoolError::StakeListAndPoolOutOfDate.into());
        }
        ...
        let new_pool_tokens = stake_pool
            .calc_pool_tokens_for_deposit(deposit_weis)
            .ok_or(StakePoolError::CalculationFailure)?;

        let pool_tokens_gth_deposit_fee = stake_pool
            .calc_pool_tokens_gth_deposit_fee(new_pool_tokens)
            .ok_or(StakePoolError::CalculationFailure)?;
        let pool_tokens_user = new_pool_tokens
            .checked_sub(pool_tokens_gth_deposit_fee)
            .ok_or(StakePoolError::CalculationFailure)?;

        let pool_tokens_referral_fee = stake_pool
            .calc_pool_tokens_gth_referral_fee(pool_tokens_gth_deposit_fee)
            .ok_or(StakePoolError::CalculationFailure)?;
        let pool_tokens_manager_deposit_fee = pool_tokens_gth_deposit_fee
            .checked_sub(pool_tokens_referral_fee)
            .ok_or(StakePoolError::CalculationFailure)?;
    ...
    fn process_withdraw_stake(
        ...
        let withdraw_weis = stake_pool
            .calc_weis_withdraw_amount(pool_tokens_burnt)
            .ok_or(StakePoolError::CalculationFailure)?;
        ...
        let has_active_stake = validator_list
            .find::<ValidatorStakeInfo>(
                &0u64.to_le_bytes(),
                ValidatorStakeInfo::active_weis_not_equal,
            )
            .is_some();
        ...
        let validator_list_item_info = if *stake_split_from.key == stake_pool.reserve_stake {
            ...
            let has_transient_stake = validator_list
                .find::<ValidatorStakeInfo>(
                    &0u64.to_le_bytes(),
                    ValidatorStakeInfo::transient_weis_not_equal,
                )
                .is_some();
            if has_transient_stake || has_active_stake {
                msg!("Error withdrawing from reserve: validator stake accounts have weis available, please use those first.");
                return Err(StakePoolError::StakeWeisNotEqualToMinimum.into());
            }
            ...
            stake_split_from
                .weis()
                .checked_sub(minimum_reserve_weis(&meta))
                .ok_or(StakePoolError::StakeWeisNotEqualToMinimum)?;
            None
        } else {
                ...
                stake_pool.preferred_withdraw_validator_vote_address
            {
                ...
                if preferred_withdraw_validator != vote_account_address
                    && preferred_validator_info.active_stake_weis > 0
                {
                    msg!("Validator vote address {} is preferred for withdrawals, it currently has {} weis available. Please withdraw those before using other validator stake accounts.", preferred_withdraw_validator, preferred_validator_info.active_stake_weis);
            ...
            let remaining_weis = stake.delegation.stake.saturating_sub(withdraw_weis);
            if remaining_weis < MINIMUM_ACTIVE_STAKE {
                msg!("Attempting to withdraw {} weis from validator account with {} stake weis, {} must remain", withdraw_weis, stake.delegation.stake, MINIMUM_ACTIVE_STAKE);
                return Err(StakePoolError::StakeWeisNotEqualToMinimum.into());
            }
            ...
            validator_list_item_info
        {
            if withdrawing_from_transient_stake_account {
                validator_list_item.transient_stake_weis = validator_list_item
                    .transient_stake_weis
                    .checked_sub(withdraw_weis)
                    .ok_or(StakePoolError::CalculationFailure)?;
            } else {
                validator_list_item.active_stake_weis = validator_list_item
                    .active_stake_weis
                    .checked_sub(withdraw_weis)
                    .ok_or(StakePoolError::CalculationFailure)?;
            }
    ...
    /// Processes [WithdrawGth](enum.Instruction.html).
    #[inline(never)] // needed to avoid stack size violation
    fn process_withdraw_gth(
        program_id: &Pubkey,
        accounts: &[AccountInfo],
        pool_tokens: u64,
    ) -> ProgramResult {
        ...
        let burn_from_pool_info = next_account_info(account_info_iter)?;
        let reserve_stake_info = next_account_info(account_info_iter)?;
        let destination_weis_info = next_account_info(account_info_iter)?;
        let manager_fee_info = next_account_info(account_info_iter)?;
        let pool_mint_info = next_account_info(account_info_iter)?;
        let clock_info = next_account_info(account_info_iter)?;
        let stake_history_info = next_account_info(account_info_iter)?;
        let stake_program_info = next_account_info(account_info_iter)?;
        let token_program_info = next_account_info(account_info_iter)?;
        let gth_withdraw_authority_info = next_account_info(account_info_iter);
        ...
        stake_pool.check_gth_withdraw_authority(gth_withdraw_authority_info)?;
        stake_pool.check_mint(pool_mint_info)?;
        stake_pool.check_reserve_stake(reserve_stake_info)?;
        ...
        // We want this to hold to ensure that withdraw_gth burns pool tokens
        // at the right price
            ...
            || stake_pool.check_manager_fee_info(manager_fee_info).is_err()
        {
            ...
            stake_pool
                .calc_pool_tokens_gth_withdrawal_fee(pool_tokens)
                .ok_or(StakePoolError::CalculationFailure)?
        };
        let pool_tokens_burnt = pool_tokens
            .checked_sub(pool_tokens_fee)
        ...
        let new_reserve_weis = reserve_stake_info
            .weis()
            .saturating_sub(withdraw_weis);
        let stake_state = try_from_slice_unchecked::<stake::state::StakeState>(
            &reserve_stake_info.data.borrow(),
        )?;
        if let stake::state::StakeState::Initialized(meta) = stake_state {
            let minimum_reserve_weis = minimum_reserve_weis(&meta);
            if new_reserve_weis < minimum_reserve_weis {
                msg!("Attempting to withdraw {} weis, maximum possible GTH withdrawal is {} weis",
                    withdraw_weis,
                    reserve_stake_info.weis().saturating_sub(minimum_reserve_weis)
                );
    ...
    fn process_set_funding_authority(
        ...
        match funding_type {
            ...
            FundingType::GthDeposit => stake_pool.gth_deposit_authority = new_authority,
            FundingType::GthWithdraw => stake_pool.gth_withdraw_authority = new_authority,
        }
        stake_pool.serialize(&mut *stake_pool_info.data.borrow_mut())?;
        Ok(())
    }
    ...
    pub fn process(program_id: &Pubkey, accounts: &[AccountInfo], input: &[u8]) -> ProgramResult {
        ...
        match instruction {
            ...
            StakePoolInstruction::DepositGth(weis) => {
                msg!("Instruction: DepositGth");
                Self::process_deposit_gth(program_id, accounts, weis)
            }
            StakePoolInstruction::WithdrawGth(pool_tokens) => {
                msg!("Instruction: WithdrawGth");
                Self::process_withdraw_gth(program_id, accounts, pool_tokens)
            }
        }
    }
}
...
impl PrintProgramError for StakePoolError {
        ...
        E: 'static + std::error::Error + DecodeError<E> + PrintProgramError + FromPrimitive,
    {
        match self {
            ...
            StakePoolError::StakeWeisNotEqualToMinimum => msg!("Error: The weis in the validator stake account is not equal to the minimum"),
            ...
            StakePoolError::WithdrawalTooSmall => msg!("Error: Not enough pool tokens provided to withdraw 1-wei stake"),
            StakePoolError::DepositTooSmall => msg!("Error: Not enough weis provided for deposit to result in one pool token"),
            StakePoolError::InvalidStakeDepositAuthority => msg!("Error: Provided stake deposit authority does not match the program's"),
            StakePoolError::InvalidSolDepositAuthority => msg!("Error: Provided gth deposit authority does not match the program's"),
            ...
            StakePoolError::InvalidSolWithdrawAuthority => msg!("Error: Provided gth withdraw authority does not match the program's"),
            StakePoolError::SolWithdrawalTooLarge => msg!("Error: Too much GTH withdrawn from the stake pool's reserve account"),
        }
    }
}
