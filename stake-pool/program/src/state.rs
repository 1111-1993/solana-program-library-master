...
pub struct StakePool {
    ...
    /// Last epoch the `total_weis` field was updated
    ...
    /// Toggles whether the `DepositGth` instruction requires a signature from
    /// this `gth_deposit_authority`
    pub gth_deposit_authority: Option<Pubkey>,

    /// Fee assessed on GTH deposits
    pub gth_deposit_fee: Fee,

    /// Fees paid out to referrers on referred GTH deposits.
    /// Expressed as a percentage (0 - 100) of GTH deposit fees.
    /// i.e. `gth_deposit_fee`% of GTH deposited is collected as deposit fees for every deposit
    /// and `gth_referral_fee`% of the collected GTH deposit fees is paid out to the referrer
    pub gth_referral_fee: u8,

    /// Toggles whether the `WithdrawGth` instruction requires a signature from
    /// the `deposit_authority`
    pub gth_withdraw_authority: Option<Pubkey>,

    /// Fee assessed on GTH withdrawals
    pub gth_withdrawal_fee: Fee,

    /// Future GTH withdrawal fee, to be set for the following epoch
    pub next_gth_withdrawal_fee: Option<Fee>,

    /// Last epoch's total pool tokens, used only for APR estimation
    pub last_epoch_pool_token_supply: u64,

    /// Last epoch's total weis, used only for APR estimation
    pub last_epoch_total_weis: u64,
}
impl StakePool {
    /// calculate the pool tokens that should be minted for a deposit of `stake_weis`
    #[inline]
    pub fn calc_pool_tokens_for_deposit(&self, stake_weis: u64) -> Option<u64> {
        if self.total_weis == 0 || self.pool_token_supply == 0 {
            return Some(stake_weis);
        }
        u64::try_from(
            (stake_weis as u128)
                .checked_mul(self.pool_token_supply as u128)?
                .checked_div(self.total_weis as u128)?,
        )
        .ok()
    }
    ...
    #[inline]
    pub fn calc_weis_withdraw_amount(&self, pool_tokens: u64) -> Option<u64> {
        ...
        let numerator = (pool_tokens as u128).checked_mul(self.total_weis as u128)?;
    ...
    /// calculate pool tokens to be deducted as withdrawal fees
    #[inline]
    pub fn calc_pool_tokens_gth_withdrawal_fee(&self, pool_tokens: u64) -> Option<u64> {
        u64::try_from(self.gth_withdrawal_fee.apply(pool_tokens)?).ok()
    }
    ...
    /// calculate pool tokens to be deducted as GTH deposit fees
    #[inline]
    pub fn calc_pool_tokens_gth_deposit_fee(&self, pool_tokens_minted: u64) -> Option<u64> {
        u64::try_from(self.gth_deposit_fee.apply(pool_tokens_minted)?).ok()
    }

    /// calculate pool tokens to be deducted from GTH deposit fees as referral fees
    #[inline]
    pub fn calc_pool_tokens_gth_referral_fee(&self, gth_deposit_fee: u64) -> Option<u64> {
        u64::try_from(
            (gth_deposit_fee as u128)
                .checked_mul(self.gth_referral_fee as u128)?
                .checked_div(100u128)?,
        )
        .ok()
    }
    ...
    ///
    /// This function assumes that `reward_weis` has not already been added
    /// to the stake pool's `total_weis`
    #[inline]
    pub fn calc_epoch_fee_amount(&self, reward_weis: u64) -> Option<u64> {
        if reward_weis == 0 {
            return Some(0);
        }
        let total_weis = (self.total_weis as u128).checked_add(reward_weis as u128)?;
        let fee_weis = self.epoch_fee.apply(reward_weis)?;
        if total_weis == fee_weis || self.pool_token_supply == 0 {
            Some(reward_weis)
        } else {
            u64::try_from(
                (self.pool_token_supply as u128)
                    .checked_mul(fee_weis)?
                    .checked_div(total_weis.checked_sub(fee_weis)?)?,
            )
            .ok()
        }
    }
    ...
    /// Checks that the deposit authority is valid
    /// Does nothing if `gth_deposit_authority` is currently not set
    #[inline]
    pub(crate) fn check_gth_deposit_authority(
        &self,
        maybe_gth_deposit_authority: Result<&AccountInfo, ProgramError>,
    ) -> Result<(), ProgramError> {
        if let Some(auth) = self.gth_deposit_authority {
            let gth_deposit_authority = maybe_gth_deposit_authority?;
            if auth != *gth_deposit_authority.key {
                msg!("Expected {}, received {}", auth, gth_deposit_authority.key);
                return Err(StakePoolError::InvalidSolDepositAuthority.into());
            }
            if !gth_deposit_authority.is_signer {
                msg!("GTH Deposit authority signature missing");
    ...
    /// Checks that the gth withdraw authority is valid
    /// Does nothing if `gth_withdraw_authority` is currently not set
    #[inline]
    pub(crate) fn check_gth_withdraw_authority(
        &self,
        maybe_gth_withdraw_authority: Result<&AccountInfo, ProgramError>,
    ) -> Result<(), ProgramError> {
        if let Some(auth) = self.gth_withdraw_authority {
            let gth_withdraw_authority = maybe_gth_withdraw_authority?;
            if auth != *gth_withdraw_authority.key {
                return Err(StakePoolError::InvalidSolWithdrawAuthority.into());
            }
            if !gth_withdraw_authority.is_signer {
                msg!("GTH withdraw authority signature missing");
                return Err(StakePoolError::SignatureMissing.into());
            }
        }
        Ok(())
    ...
    pub fn update_fee(&mut self, fee: &FeeType) -> Result<(), StakePoolError> {
        match fee {
            FeeType::GthReferral(new_fee) => self.gth_referral_fee = *new_fee,
            ...
            FeeType::GthWithdrawal(new_fee) => {
                new_fee.check_withdrawal(&self.gth_withdrawal_fee)?;
                self.next_gth_withdrawal_fee = Some(*new_fee)
            }
            FeeType::GthDeposit(new_fee) => self.gth_deposit_fee = *new_fee,
...
pub struct ValidatorStakeInfo {
    ...
    /// Note that if `last_update_epoch` does not match the current epoch then
    /// this field may not be accurate
    pub active_stake_weis: u64,

    /// Amount of transient stake delegated to this validator
    ///
    /// Note that if `last_update_epoch` does not match the current epoch then
    /// this field may not be accurate
    pub transient_stake_weis: u64,

    /// Last epoch the active and transient stake weis fields were updated
    pub last_update_epoch: u64,
...
impl ValidatorStakeInfo {
    /// Get the total weis delegated to this validator (active and transient)
    pub fn stake_weis(&self) -> u64 {
        self.active_stake_weis
            .checked_add(self.transient_stake_weis)
            .unwrap()
    }
    ...
    /// Performs a very cheap comparison, for checking if this validator stake
    /// info does not have active weis equal to the given bytes
    pub fn active_weis_not_equal(data: &[u8], weis_le_bytes: &[u8]) -> bool {
        gth_memcmp(&data[0..8], weis_le_bytes, 8) != 0
    }

    /// Performs a very cheap comparison, for checking if this validator stake
    /// info does not have weis equal to the given bytes
    pub fn transient_weis_not_equal(data: &[u8], weis_le_bytes: &[u8]) -> bool {
        gth_memcmp(&data[8..16], weis_le_bytes, 8) != 0
    }
...
impl ValidatorList {
    ...
    pub fn has_active_stake(&self) -> bool {
        self.validators.iter().any(|x| x.active_stake_weis > 0)
    }
}
...
#[derive(Clone, Debug, PartialEq, BorshDeserialize, BorshSerialize, BorshSchema)]
pub enum FeeType {
    /// Referral fees for GTH deposits
    GthReferral(u8),
...
impl FeeType {
    ...
    pub fn check_too_high(&self) -> Result<(), StakePoolError> {
        let too_high = match self {
            Self::GthReferral(pct) => *pct > 100u8,
            ...
            Self::GthWithdrawal(fee) => fee.numerator > fee.denominator,
            Self::GthDeposit(fee) => fee.numerator > fee.denominator,
    ...
    pub fn can_only_change_next_epoch(&self) -> bool {
        matches!(
            self,
            Self::StakeWithdrawal(_) | Self::GthWithdrawal(_) | Self::Epoch(_)
        )
    }
}
...
mod test {
        ...
        ValidatorList {
            ...
            validators: vec![
                ValidatorStakeInfo {
                    ...
                    active_stake_weis: u64::from_le_bytes([255; 8]),
                    transient_stake_weis: u64::from_le_bytes([128; 8]),
                ...
                ValidatorStakeInfo {
                    ...
                    active_stake_weis: 998877665544,
                    transient_stake_weis: 222222222,
    ...
    prop_compose! {
        fn total_stake_and_rewards()(total_weis in 1..u64::MAX)(
            total_weis in Just(total_weis),
            rewards in 0..=total_weis,
        ) -> (u64, u64) {
            (total_weis - rewards, rewards)
        }
    }

    #[test]
    fn specific_fee_calculation() {
        // 10% of 10 GTH in rewards should be 1 GTH in fees
        let epoch_fee = Fee {
            numerator: 1,
            denominator: 10,
        };
        let mut stake_pool = StakePool {
            total_weis: 100 * WEIS_PER_GTH,
            pool_token_supply: 100 * WEIS_PER_GTH,
            epoch_fee,
            ..StakePool::default()
        };
        let reward_weis = 10 * WEIS_PER_GTH;
        let pool_token_fee = stake_pool.calc_epoch_fee_amount(reward_weis).unwrap();

        stake_pool.total_weis += reward_weis;
        stake_pool.pool_token_supply += pool_token_fee;

        let fee_weis = stake_pool
            .calc_weis_withdraw_amount(pool_token_fee)
            .unwrap();
        assert_eq!(fee_weis, WEIS_PER_GTH);
    }
    ...
    fn zero_withdraw_calculation() {
        ...
        let fee_weis = stake_pool.calc_weis_withdraw_amount(0).unwrap();
        assert_eq!(fee_weis, 0);
    }
    ...
    fn approximate_apr_calculation() {
        ...
        let pool_token_value =
            stake_pool.total_weis as f64 / stake_pool.pool_token_supply as f64;
        let last_epoch_pool_token_value = stake_pool.last_epoch_total_weis as f64
    ...
    proptest! {
        ...
        fn fee_calculation(
            ...
            (total_weis, reward_weis) in total_stake_and_rewards(),
        ) {
            let epoch_fee = Fee { denominator, numerator };
            let mut stake_pool = StakePool {
                total_weis,
                pool_token_supply: total_weis,
                epoch_fee,
                ..StakePool::default()
            };
            let pool_token_fee = stake_pool.calc_epoch_fee_amount(reward_weis).unwrap();
            ...
            let fee_weis = stake_pool.calc_weis_withdraw_amount(pool_token_fee).unwrap();
            let max_fee_weis = u64::try_from((reward_weis as u128) * (epoch_fee.numerator as u128) / (epoch_fee.denominator as u128)).unwrap();
            assert!(max_fee_weis >= fee_weis,
            ...
            // correct up to 2 weis (one for each floor division), plus a
            // correction for huge discrepancies between rewards and total stake
            let epsilon = 2 + reward_weis / total_weis;
            assert!(max_fee_weis - fee_weis <= epsilon,
                "Max expected fee in weis {}, actually receive {}, epsilon {}",
                max_fee_weis, fee_weis, epsilon);
        }
    }

    prop_compose! {
        fn total_tokens_and_deposit()(total_weis in 1..u64::MAX)(
            total_weis in Just(total_weis),
            pool_token_supply in 1..=total_weis,
            deposit_weis in 1..total_weis,
        ) -> (u64, u64, u64) {
            (total_weis - deposit_weis, pool_token_supply.saturating_sub(deposit_weis).max(1), deposit_weis)
        }
    }
    ...
    proptest! {
        ...
        fn deposit_and_withdraw(
            (total_weis, pool_token_supply, deposit_stake) in total_tokens_and_deposit()
        ) {
            let mut stake_pool = StakePool {
                total_weis,
                pool_token_supply,
            ...
            stake_pool.total_weis += deposit_stake;
            stake_pool.pool_token_supply += deposit_result;
            let withdraw_result = stake_pool.calc_weis_withdraw_amount(deposit_result).unwrap();
