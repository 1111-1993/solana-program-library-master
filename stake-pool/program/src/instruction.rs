...
pub enum FundingType {
    ...
    StakeDeposit,
    /// Sets the GTH deposit authority
    GthDeposit,
    /// Sets the GTH withdraw authority
    GthWithdraw,
}
...
pub enum StakePoolInstruction {
    ...
    ///   `crate::MINIMUM_ACTIVE_STAKE` (currently 0.001 GTH).
    ///
    ///   0. `[w]` Stake pool
    ///   1. `[s]` Staker
    ...
    ///   `crate::MINIMUM_ACTIVE_STAKE` (currently 0.001 GTH) plus the rent-exempt
    ///   amount.
    ///
    ///   0. `[w]` Stake pool
    ///   1. `[s]` Staker
    ...
    ///   7. `[w]` Destination stake account, to receive the minimum GTH from the validator stake account
    ///   8. `[]` Sysvar clock
    ///   9. `[]` Stake program id,
    ...
    /// exist. The amount of weis to move must be at least rent-exemption
    /// plus 1 wei.
    ///
    ///  0. `[]` Stake pool
    ...
    DecreaseValidatorStake {
        /// amount of weis to split into the transient stake account
        #[allow(dead_code)] // but it's not
        weis: u64,
    ...
    /// (currently 0.001 GTH) in order to avoid issues on credits observed when
    ...
    /// between GTH staked on different validators, the staker can force all
    ...
    ///
    ///   Succeeds if the stake account has enough GTH to cover the desired amount
    ///   of pool tokens, and if the withdrawal keeps the total staked amount
    ///   above the minimum of rent-exempt amount + 0.001 GTH.
    ///
    ...
    ///   Deposit GTH directly into the pool's reserve account. The output is a "pool" token
    ///   representing ownership into the pool. Inputs are converted to the current ratio.
    ///
    ///   0. `[w]` Stake pool
    ///   1. `[]` Stake pool withdraw authority
    ///   2. `[w]` Reserve stake account, to deposit GTH
    ///   3. `[s]` Account providing the weis to be deposited into the pool
    ...
    ///   9. `[]` Token program id
    ///  10. `[s]` (Optional) Stake pool gth deposit authority.
    DepositGth(u64),

    ///  (Manager only) Update GTH deposit authority
    ///
    ///  0. `[w]` StakePool
    ///  1. `[s]` Manager
    ...
    ///   Withdraw GTH directly from the pool's reserve account. Fails if the
    ///   reserve does not have enough GTH.
    ///
    ///   0. `[w]` Stake pool
    ///   1. `[]` Stake pool withdraw authority
    ///   2. `[s]` User transfer authority, for pool token account
    ///   3. `[w]` User account to burn pool tokens
    ///   4. `[w]` Reserve stake account, to withdraw GTH
    ///   5. `[w]` Account receiving the weis from the reserve, must be a system account
    ...
    ///  11. `[]` Token program id
    ///  12. `[s]` (Optional) Stake pool gth withdraw authority
    WithdrawGth(u64),
}
...
/// Creates instructions required to deposit GTH directly into a stake pool.
pub fn deposit_gth(
    program_id: &Pubkey,
    stake_pool: &Pubkey,
    stake_pool_withdraw_authority: &Pubkey,
    ...
    let accounts = vec![
        ...
        AccountMeta::new_readonly(*stake_pool_withdraw_authority, false),
        AccountMeta::new(*reserve_stake_account, false),
        AccountMeta::new(*weis_from, true),
        AccountMeta::new(*pool_tokens_to, false),
        AccountMeta::new(*manager_fee_account, false),
        AccountMeta::new(*referrer_pool_tokens_account, false),
        AccountMeta::new(*pool_mint, false),
    ...
    Instruction {
        program_id: *program_id,
        accounts,
        data: StakePoolInstruction::DepositGth(amount)
            .try_to_vec()
            .unwrap(),
    }
}

/// Creates instruction required to deposit GTH directly into a stake pool.
/// The difference with `deposit_gth()` is that a deposit
/// authority must sign this instruction.
pub fn deposit_gth_with_authority(
    program_id: &Pubkey,
    stake_pool: &Pubkey,
    gth_deposit_authority: &Pubkey,
    stake_pool_withdraw_authority: &Pubkey,
    reserve_stake_account: &Pubkey,
    weis_from: &Pubkey,
    pool_tokens_to: &Pubkey,
    ...
    let accounts = vec![
        ...
        AccountMeta::new_readonly(system_program::id(), false),
        AccountMeta::new_readonly(*token_program_id, false),
        AccountMeta::new_readonly(*gth_deposit_authority, true),
    ];
    Instruction {
        program_id: *program_id,
        accounts,
...
/// Creates instruction required to withdraw GTH directly from a stake pool.
pub fn withdraw_gth(
    program_id: &Pubkey,
    stake_pool: &Pubkey,
    stake_pool_withdraw_authority: &Pubkey,
    ...
    Instruction {
        program_id: *program_id,
        accounts,
        data: StakePoolInstruction::WithdrawGth(pool_tokens)
            .try_to_vec()
            .unwrap(),
    }
}
...
/// Creates instruction required to withdraw GTH directly from a stake pool.
/// The difference with `withdraw_gth()` is that the gth withdraw authority
/// must sign this instruction.
pub fn withdraw_gth_with_authority(
    program_id: &Pubkey,
    stake_pool: &Pubkey,
    gth_withdraw_authority: &Pubkey,
    stake_pool_withdraw_authority: &Pubkey,
    user_transfer_authority: &Pubkey,
    pool_tokens_from: &Pubkey,
    reserve_stake_account: &Pubkey,
    ...
    let accounts = vec![
        ...
        AccountMeta::new_readonly(stake::program::id(), false),
        AccountMeta::new_readonly(*token_program_id, false),
        AccountMeta::new_readonly(*gth_withdraw_authority, true),
    ];
    Instruction {
        program_id: *program_id,
        accounts,
...
pub fn set_funding_authority(
    ...
    stake_pool: &Pubkey,
    manager: &Pubkey,
    new_gth_deposit_authority: Option<&Pubkey>,
    ...
    if let Some(auth) = new_gth_deposit_authority {
