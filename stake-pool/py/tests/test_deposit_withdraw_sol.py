...
from stake_pool.actions import create_all, deposit_gth, withdraw_gth


@pytest.mark.asyncio
async def test_deposit_withdraw_gth(async_client, payer):
    ...
    recipient = Keypair()
    await withdraw_gth(async_client, payer, token_account, stake_pool_address, recipient.public_key, deposit_amount)
    pool_token_balance = await async_client.get_token_account_balance(token_account, Confirmed)
