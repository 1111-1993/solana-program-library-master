...
from stake_pool.actions import deposit_gth, decrease_validator_stake, increase_validator_stake, update_stake_pool
...
async def test_increase_decrease_this_is_very_slow(async_client, validators, payer, stake_pool_addresses, waiter):
    ...
    for validator in validator_list.validators:
        ...
        assert validator.active_stake_weis == increase_amount  # rent exemption brought back to reserve
    ...
    validator_list = ValidatorList.decode(data[0], data[1])
    for validator in validator_list.validators:
        assert validator.transient_stake_weis == decrease_amount
        assert validator.active_stake_weis == increase_amount - decrease_amount
