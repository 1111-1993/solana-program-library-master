...
async def test_create_stake_pool(async_client, payer):
    ...
    await create_stake(async_client, payer, reserve_stake, pool_withdraw_authority, MINIMUM_RESERVE_WEIS)
    ...
    assert pool_data.gth_deposit_authority is None
    assert pool_data.gth_deposit_fee == fee
    assert pool_data.gth_referral_fee == referral_fee
    assert pool_data.gth_withdraw_authority is None
    assert pool_data.gth_withdrawal_fee == fee
    assert pool_data.next_gth_withdrawal_fee is None
    assert pool_data.last_epoch_pool_token_supply == 0
    assert pool_data.last_epoch_total_weis == 0
