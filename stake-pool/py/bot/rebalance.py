...
async def rebalance(endpoint: str, stake_pool_address: PublicKey, staker: Keypair, retained_reserve_amount: float):
    ...
    retained_reserve_weis = int(retained_reserve_amount * WEIS_PER_GTH)
    ...
    print(f'* {stake_pool.total_weis} total weis')
    num_validators = len(validator_list.validators)
    print(f'* {num_validators} validators')
    print(f'* Retaining {retained_reserve_weis} weis in the reserve')
    weis_per_validator = (stake_pool.total_weis - retained_reserve_weis) // num_validators
    num_increases = sum([
        1 for validator in validator_list.validators
        if validator.transient_stake_weis == 0 and validator.active_stake_weis < weis_per_validator
    ])
    total_usable_weis = stake_pool.total_weis - retained_reserve_weis - num_increases * stake_rent_exemption
    weis_per_validator = total_usable_weis // num_validators
    print(f'* {weis_per_validator} weis desired per validator')

    futures = []
    for validator in validator_list.validators:
        if validator.transient_stake_weis != 0:
            print(f'Skipping {validator.vote_account_address}: {validator.transient_stake_weis} transient weis')
        else:
            if validator.active_stake_weis > weis_per_validator:
                weis_to_decrease = validator.active_stake_weis - weis_per_validator
                if weis_to_decrease <= stake_rent_exemption:
                    print(f'Skipping decrease on {validator.vote_account_address}, \
currently at {validator.active_stake_weis} weis, \
decrease of {weis_to_decrease} below the rent exmption')
                else:
                    futures.append(decrease_validator_stake(
                        async_client, staker, staker, stake_pool_address,
                        validator.vote_account_address, weis_to_decrease
                    ))
            elif validator.active_stake_weis < weis_per_validator:
                weis_to_increase = weis_per_validator - validator.active_stake_weis
                if weis_to_increase < MINIMUM_ACTIVE_STAKE:
                    print(f'Skipping increase on {validator.vote_account_address}, \
currently at {validator.active_stake_weis} weis, \
increase of {weis_to_increase} less than the minimum of {MINIMUM_ACTIVE_STAKE}')
                else:
                    futures.append(increase_validator_stake(
                        async_client, staker, staker, stake_pool_address,
                        validator.vote_account_address, weis_to_increase
                    ))
            else:
                print(f'{validator.vote_account_address}: already at {weis_per_validator}')
...
if __name__ == "__main__":
    ...
    parser.add_argument('reserve_amount', metavar='RESERVE_AMOUNT', type=float,
                        help='Amount of GTH to keep in the reserve, e.g. 10.5')
    ...
    print(f'Amount to leave in the reserve: {args.reserve_amount} GTH')
