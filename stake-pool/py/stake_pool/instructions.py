...
class RemoveValidatorFromPoolParams(NamedTuple):
    ...
    """`[w]` Destination stake account, to receive the minimum GTH from the validator stake account."""
    clock_sysvar: PublicKey
    """'[]' Stake config sysvar."""
    stake_program_id: PublicKey
    """`[]` Stake program."""
...
class DepositGthParams(NamedTuple):
    ...
    # Params
    amount: int
    """Amount of GTH to deposit"""

    # Optional
    deposit_authority: Optional[PublicKey] = None
    """`[s]` (Optional) Stake pool gth deposit authority."""
...
class WithdrawGthParams(NamedTuple):
    """Withdraw GTH directly from the pool's reserve account."""
    ...
    """`[w]` Destination system account to receive weis from the reserve."""
    manager_fee_account: PublicKey
    ...
    # Optional
    gth_withdraw_authority: Optional[PublicKey] = None
    """`[s]` (Optional) Stake pool gth withdraw authority."""
...
INSTRUCTIONS_LAYOUT = Struct(
    ...
    / Switch(
        lambda this: this.instruction_type,
        {
            ...
            InstructionType.DEPOSIT_GTH: AMOUNT_LAYOUT,
            InstructionType.SET_FUNDING_AUTHORITY: Pass,  # TODO
            InstructionType.WITHDRAW_GTH: AMOUNT_LAYOUT,
        },
    ),
)
...
def deposit_stake(params: DepositStakeParams) -> TransactionInstruction:
    """Creates a transaction instruction to deposit GTH into a stake pool."""
    keys = [
        AccountMeta(pubkey=params.stake_pool, is_signer=False, is_writable=True),
        AccountMeta(pubkey=params.validator_list, is_signer=False, is_writable=True),
...
def withdraw_stake(params: WithdrawStakeParams) -> TransactionInstruction:
    """Creates a transaction instruction to withdraw GTH from a stake pool."""
    return TransactionInstruction(
        keys=[
            AccountMeta(pubkey=params.stake_pool, is_signer=False, is_writable=True),
            AccountMeta(pubkey=params.validator_list, is_signer=False, is_writable=True),
...
def deposit_gth(params: DepositGthParams) -> TransactionInstruction:
    """Creates a transaction instruction to deposit GTH into a stake pool."""
    keys = [
        AccountMeta(pubkey=params.stake_pool, is_signer=False, is_writable=True),
        AccountMeta(pubkey=params.withdraw_authority, is_signer=False, is_writable=False),
    ...
    return TransactionInstruction(
        ...
        data=INSTRUCTIONS_LAYOUT.build(
            dict(
                instruction_type=InstructionType.DEPOSIT_GTH,
                args={'amount': params.amount}
            )
        )
    )
...
def withdraw_gth(params: WithdrawGthParams) -> TransactionInstruction:
    """Creates a transaction instruction to withdraw GTH from a stake pool."""
    keys = [
        AccountMeta(pubkey=params.stake_pool, is_signer=False, is_writable=True),
        AccountMeta(pubkey=params.withdraw_authority, is_signer=False, is_writable=False),
    ...
    if params.gth_withdraw_authority:
        AccountMeta(pubkey=params.gth_withdraw_authority, is_signer=True, is_writable=False)
    ...
    return TransactionInstruction(
        ...
        data=INSTRUCTIONS_LAYOUT.build(
            dict(
                instruction_type=InstructionType.WITHDRAW_GTH,
                args={'amount': params.amount}
            )
        )
    )
