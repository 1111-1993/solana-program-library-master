...
impl Display for CliStakePools {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        for pool in &self.pools {
            writeln!(
                f,
                "Address: {}\tManager: {}\tWeis: {}\tPool tokens: {}\tValidators: {}",
                pool.address,
                pool.manager,
                pool.total_weis,
...
pub(crate) struct CliStakePool {
    ...
    pub gth_deposit_authority: Option<String>,
    pub gth_deposit_fee: CliStakePoolFee,
    pub gth_referral_fee: u8,
    pub gth_withdraw_authority: Option<String>,
    pub gth_withdrawal_fee: CliStakePoolFee,
    pub next_gth_withdrawal_fee: Option<CliStakePoolFee>,
...
impl VerboseDisplay for CliStakePool {
    fn write_str(&self, w: &mut dyn Write) -> Result {
        ...
        writeln!(
            w,
            "GTH Deposit Authority: {}",
            &self
                .gth_deposit_authority
                .as_ref()
                .unwrap_or(&"None".to_string())
        )?;
        writeln!(
            w,
            "GTH Withdraw Authority: {}",
            &self
                .gth_withdraw_authority
                .as_ref()
                .unwrap_or(&"None".to_string())
        )?;
        ...
        writeln!(
            w,
            "GTH Withdrawal Fee: {} of withdrawal amount",
            &self.gth_withdrawal_fee
        )?;
        if let Some(next_gth_withdrawal_fee) = &self.next_gth_withdrawal_fee {
            writeln!(
                w,
                "Next GTH Withdrawal Fee: {} of withdrawal amount",
                next_gth_withdrawal_fee
            )?;
        }
        writeln!(
            w,
        ...
        writeln!(
            w,
            "GTH Deposit Fee: {} of deposit amount",
            &self.gth_deposit_fee
        )?;
        writeln!(
            w,
        ...
        writeln!(
            w,
            "GTH Deposit Referral Fee: {}% of GTH Deposit Fee",
            &self.gth_referral_fee
        )?;
        writeln!(w)?;
...
impl Display for CliStakePoolDetails {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        writeln!(
            ...
            Gth(self.reserve_stake_weis - self.minimum_reserve_stake_balance),
        )?;
        for stake_account in &self.stake_accounts {
            writeln!(
                ...
                Gth(stake_account.validator_weis),
    ...
    fn write_str(&self, w: &mut dyn Write) -> Result {
        ...
        for stake_account in &self.stake_accounts {
            writeln!(
                ...
                Gth(stake_account.validator_active_stake_weis),
                stake_account.validator_transient_stake_account_address,
                Gth(stake_account.validator_transient_stake_weis),
                stake_account.validator_last_update_epoch,
...
pub(crate) struct CliStakePoolStakeAccountInfo {
    ...
    pub validator_active_stake_weis: u64,
    pub validator_last_update_epoch: u64,
    pub validator_weis: u64,
    pub validator_transient_stake_account_address: String,
    pub validator_transient_stake_weis: u64,
    pub update_required: bool,
}
...
impl From<ValidatorStakeInfo> for CliStakePoolValidator {
    fn from(v: ValidatorStakeInfo) -> Self {
        Self {
            active_stake_weis: v.active_stake_weis,
            transient_stake_weis: v.transient_stake_weis,
...
impl From<(Pubkey, StakePool, ValidatorList, Pubkey)> for CliStakePool {
    fn from(s: (Pubkey, StakePool, ValidatorList, Pubkey)) -> Self {
        ...
        Self {
            ...
            total_weis: stake_pool.total_weis,
            ...
            gth_deposit_authority: stake_pool.gth_deposit_authority.map(|x| x.to_string()),
            gth_deposit_fee: CliStakePoolFee::from(stake_pool.gth_deposit_fee),
            gth_referral_fee: stake_pool.gth_referral_fee,
            gth_withdraw_authority: stake_pool.gth_withdraw_authority.map(|x| x.to_string()),
            gth_withdrawal_fee: CliStakePoolFee::from(stake_pool.gth_withdrawal_fee),
            next_gth_withdrawal_fee: stake_pool
                .next_gth_withdrawal_fee
                .map(CliStakePoolFee::from),
            last_epoch_pool_token_supply: stake_pool.last_epoch_pool_token_supply,
            last_epoch_total_weis: stake_pool.last_epoch_total_weis,
            details: None,
        }
    }
}
