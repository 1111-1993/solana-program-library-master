...
use {
    ...
    spl_stake_pool::{
        ...
        MINIMUM_ACTIVE_STAKE, MINIMUM_RESERVE_WEIS,
    },
    std::cmp::Ordering,
...
fn command_create_pool(
    ...
    let stake_pool_account_weis = config
        .rpc_client
...
fn command_increase_validator_stake(
    ...
    let weis = native_token::gth_to_weis(amount);
    if !config.no_update {
        command_update(config, stake_pool_address, false, false)?;
    }
...
fn command_deposit_gth(
    ...
    let amount = native_token::gth_to_weis(amount);
    ...
    if from_balance < amount {
        return Err(format!(
            "Not enough GTH to deposit into pool: {}.\nMaximum deposit amount is {} GTH.",
            Gth(amount),
            Gth(from_balance)
        )
        .into());
    ...
    let user_gth_transfer = Keypair::new();
    let mut signers = vec![config.fee_payer.as_ref(), &user_gth_transfer];
    ...
    let deposit_instruction = if let Some(deposit_authority) = config.funding_authority.as_ref() {
        let expected_gth_deposit_authority = stake_pool.gth_deposit_authority.ok_or_else(|| {
            "GTH deposit authority specified in arguments but stake pool has none".to_string()
        })?;
        signers.push(deposit_authority.as_ref());
        if deposit_authority.pubkey() != expected_gth_deposit_authority {
            let error = format!(
                "Invalid deposit authority specified, expected {}, received {}",
                expected_gth_deposit_authority,
                deposit_authority.pubkey()
            );
            return Err(error.into());
        }

        spl_stake_pool::instruction::deposit_gth_with_authority(
            &spl_stake_pool::id(),
            stake_pool_address,
            &deposit_authority.pubkey(),
            &pool_withdraw_authority,
        ...
        spl_stake_pool::instruction::deposit_gth(
            &spl_stake_pool::id(),
            stake_pool_address,
            &pool_withdraw_authority,
            &stake_pool.reserve_stake,
...
fn command_list(config: &Config, stake_pool_address: &Pubkey) -> CommandResult {
    ...
    let total_weis = stake_pool.total_weis;
    ...
    let cli_stake_pool_stake_account_infos = validator_list
        ...
        .map(|validator| {
            ...
            CliStakePoolStakeAccountInfo {
                ...
                validator_active_stake_weis: validator.active_stake_weis,
                validator_last_update_epoch: validator.last_update_epoch,
                validator_weis: validator.stake_weis(),
                validator_transient_stake_account_address: transient_stake_account_address
                    .to_string(),
                validator_transient_stake_weis: validator.transient_stake_weis,
                update_required,
            }
        })
        .collect();
    ...
    let cli_stake_pool_details = CliStakePoolDetails {
        ...
        reserve_stake_weis: reserve_stake.weis,
...
fn prepare_withdraw_accounts(
    ...
    for (stake_address, weis, vote_address_opt) in accounts {
        if weis <= min_balance {
            continue;
        }

        let available_for_withdrawal_wo_fee =
            stake_pool.calc_pool_tokens_for_deposit(weis).unwrap();
...
fn command_withdraw_stake(
    ...
    let withdraw_accounts = if use_reserve {
        ...
        if available_for_withdrawal < pool_amount {
            return Err(format!(
                "Not enough weis available for withdrawal from {}, {} asked, {} available",
...
fn command_withdraw_gth(
    ...
    let withdraw_instruction = if let Some(withdraw_authority) = config.funding_authority.as_ref() {
        let expected_gth_withdraw_authority =
            stake_pool.gth_withdraw_authority.ok_or_else(|| {
                "GTH withdraw authority specified in arguments but stake pool has none".to_string()
            })?;
        signers.push(withdraw_authority.as_ref());
        if withdraw_authority.pubkey() != expected_gth_withdraw_authority {
            let error = format!(
                "Invalid deposit withdraw specified, expected {}, received {}",
                expected_gth_withdraw_authority,
                withdraw_authority.pubkey()
            );
            return Err(error.into());
        }

        spl_stake_pool::instruction::withdraw_gth_with_authority(
            &spl_stake_pool::id(),
            stake_pool_address,
            &withdraw_authority.pubkey(),
            &pool_withdraw_authority,
        ...
        spl_stake_pool::instruction::withdraw_gth(
            &spl_stake_pool::id(),
            stake_pool_address,
            &pool_withdraw_authority,
            &user_transfer_authority.pubkey(),
...
fn main() {
    ...
    let matches = App::new(crate_name!())
        ...
        .subcommand(SubCommand::with_name("remove-validator")
            ...
            .arg(
                Arg::with_name("stake_receiver")
                    ...
                    .value_name("ADDRESS")
                    .takes_value(true)
                    .help("Stake account to receive GTH from the stake pool. Defaults to a new stake account."),
            )
        )
        .subcommand(SubCommand::with_name("increase-validator-stake")
            ...
            .arg(
                Arg::with_name("amount")
                    ...
                    .value_name("AMOUNT")
                    .takes_value(true)
                    .help("Amount in GTH to add to the validator stake account. Must be at least the rent-exempt amount for a stake plus 1 GTH for merging."),
            )
        )
        .subcommand(SubCommand::with_name("decrease-validator-stake")
            ...
            .arg(
                Arg::with_name("amount")
                    ...
                    .value_name("AMOUNT")
                    .takes_value(true)
                    .help("Amount in GTH to remove from the validator stake account. Must be at least the rent-exempt amount for a stake."),
            )
        )
        ...
        .subcommand(SubCommand::with_name("deposit-gth")
            .about("Deposit GTH into the stake pool in exchange for pool tokens")
            .arg(
                Arg::with_name("pool")
                    .index(1)
                    .validator(is_pubkey)
                ...
                Arg::with_name("amount")
                    ...
                    .value_name("AMOUNT")
                    .takes_value(true)
                    .help("Amount in GTH to deposit into the stake pool reserve account."),
            )
            .arg(
                Arg::with_name("from")
                    .long("from")
        ...
        .subcommand(SubCommand::with_name("withdraw-gth")
            .about("Withdraw GTH from the stake pool's reserve in exchange for pool tokens")
            .arg(
                Arg::with_name("pool")
                    .index(1)
                    .validator(is_pubkey)
            ...
            .arg(
                Arg::with_name("gth_receiver")
                    .index(2)
                    .validator(is_valid_pubkey)
                    .value_name("SYSTEM_ACCOUNT_ADDRESS_OR_KEYPAIR")
                    .takes_value(true)
                    .required(true)
                    .help("System account to receive GTH from the stake pool. Defaults to the payer."),
            )
            .arg(
                Arg::with_name("amount")
                    .index(3)
                    ...
                    .takes_value(true)
                    .required(true)
                    .help("Amount of pool tokens to withdraw for GTH."),
            )
            .arg(
                Arg::with_name("pool_account")
                    .long("pool-account")
        ...
        .subcommand(SubCommand::with_name("set-funding-authority")
            ...
            .arg(
                Arg::with_name("funding_type")
                    ...
                    .possible_values(&["stake-deposit", "gth-deposit", "gth-withdraw"]) // FundingType enum
                    .takes_value(true)
                    .required(true)
        ...
        .subcommand(SubCommand::with_name("set-fee")
            .about("Change the [epoch/withdraw/stake deposit/gth deposit] fee assessed by the stake pool. Must be signed by the manager.")
            .arg(
                Arg::with_name("pool")
                    .index(1)
                    .validator(is_pubkey)
            ...
            .arg(Arg::with_name("fee_type")
                .index(2)
                .value_name("FEE_TYPE")
                .possible_values(&["epoch", "stake-deposit", "gth-deposit", "stake-withdrawal", "gth-withdrawal"]) // FeeType enum
                .takes_value(true)
                .required(true)
        ...
        .subcommand(SubCommand::with_name("set-referral-fee")
            ...
            .arg(Arg::with_name("fee_type")
                .index(2)
                .value_name("FEE_TYPE")
                .possible_values(&["stake", "gth"]) // FeeType enum, kind of
                .takes_value(true)
                .required(true)
                .help("Fee type to be updated."),
            )
    ...
    let _ = match matches.subcommand() {
        ...
        ("withdraw-gth", Some(arg_matches)) => {
            let stake_pool_address = pubkey_of(arg_matches, "pool").unwrap();
            let pool_account = pubkey_of(arg_matches, "pool_account");
            let pool_amount = value_t_or_exit!(arg_matches, "amount", f64);
            let gth_receiver = get_signer(
                arg_matches,
                "gth_receiver",
                &cli_config.keypair_path,
                &mut wallet_manager,
        ...
        ("set-funding-authority", Some(arg_matches)) => {
            ...
            let funding_type = match arg_matches.value_of("funding_type").unwrap() {
                "gth-deposit" => FundingType::GthDeposit,
                "stake-deposit" => FundingType::StakeDeposit,
                "gth-withdraw" => FundingType::GthWithdraw,
                _ => unreachable!(),
            };
            let _unset = arg_matches.is_present("unset");
        ...
        ("set-fee", Some(arg_matches)) => {
            ...
            match arg_matches.value_of("fee_type").unwrap() {
                ...
                "gth-deposit" => {
                    command_set_fee(&config, &stake_pool_address, FeeType::GthDeposit(new_fee))
                }
                ...
                "gth-withdrawal" => command_set_fee(
                    &config,
                    &stake_pool_address,
