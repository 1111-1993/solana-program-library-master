...
# Script to add a certain amount of GTH into a stake pool, given the stake pool
...
increase_stakes () {
  ...
  do
    $spl_stake_pool increase-validator-stake "$stake_pool_pubkey" "$validator" "$gth_amount"
  done < "$validator_list"
}

stake_pool_pubkey=$(solana-keygen pubkey "$stake_pool_keyfile")
echo "Increasing amount delegated to each validator in stake pool"
increase_stakes "$stake_pool_pubkey" "$validator_list" "$gth_amount"
