...
# Script to deposit stakes and GTH into a stake pool, given the stake pool keyfile
...
create_user_stakes () {
  ...
  do
    ...
    solana create-stake-account "$keys_dir/stake_$validator.json" "$gth_amount" --withdraw-authority "$authority" --stake-authority "$authority"
  done < "$validator_list"
}
...
create_user_stakes "$validator_list" "$gth_amount" $authority
...
echo "Depositing GTH into stake pool"
$spl_stake_pool deposit-gth "$stake_pool_pubkey" "$gth_amount"
