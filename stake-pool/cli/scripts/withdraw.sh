...
# Script to withdraw stakes and GTH from a stake pool, given the stake pool public key
# and a path to a file containing a list of validator vote accounts

cd "$(dirname "$0")" || exit
stake_pool_keyfile=$1
validator_list=$2
withdraw_gth_amount=$3
...
withdraw_stakes "$stake_pool_pubkey" "$validator_list" "$withdraw_gth_amount"
echo "Withdrawing GTH from stake pool to authority"
$spl_stake_pool withdraw-gth "$stake_pool_pubkey" $authority "$withdraw_gth_amount"
