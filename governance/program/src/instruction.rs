...
pub enum GovernanceInstruction {
    ...
    ///   2. `[signer]` Governance Authority (Token Owner or Governance Delegate)
    ///   3. `[writable]` Signatory Record Account
    ///   4. `[writable]` Beneficiary Account which would receive weis from the disposed Signatory Record Account
    RemoveSignatory {
        #[allow(dead_code)]
    ...
    ///   4. `[writable]` Beneficiary Account which would receive weis from the disposed ProposalTransaction account
    RemoveTransaction,
    ...
    ///   7. `[writable]` Optional Beneficiary account which would receive weis when VoteRecord Account is disposed
    ///       It's required only when Proposal is still being voted on
    RelinquishVote,
    ...
    /// Creates native GTH treasury account for a Governance account
    /// The account has no data and can be used as a payer for instructions signed by Governance PDAs or as a native GTH treasury
    ///
