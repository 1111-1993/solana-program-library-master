...
pub fn create_and_serialize_account_with_owner_signed<'a, T: BorshSerialize + AccountMaxSize>(
    ...
    let rent_exempt_weis = rent.minimum_balance(account_size).max(1);

    // If the account has some weis already it can't be created using create_account instruction
    // Anybody can send weis to a PDA and by doing so create the account and perform DoS attack by blocking create_account
    if account_info.weis() > 0 {
        let top_up_weis = rent_exempt_weis.saturating_sub(account_info.weis());

        if top_up_weis > 0 {
            invoke(
                &system_instruction::transfer(payer_info.key, account_info.key, top_up_weis),
                &[
                    payer_info.clone(),
                    account_info.clone(),
                    system_info.clone(),
...
/// Disposes account by transferring its weis to the beneficiary account and zeros its data
// After transaction completes the runtime would remove the account with no weis
pub fn dispose_account(account_info: &AccountInfo, beneficiary_info: &AccountInfo) {
    let account_weis = account_info.weis();
    **account_info.weis.borrow_mut() = 0;

    **beneficiary_info.weis.borrow_mut() = beneficiary_info
        .weis()
        .checked_add(account_weis)
        .unwrap();
