...
async fn test_name_service() {
    ...
    let create_name_instruction = create(
        ...
        NameRegistryInstruction::Create {
            ...
            weis: rent.minimum_balance(space + NameRecordHeader::LEN),
            space: space as u32,
        },
    ...
    let name = "bonfida";
    let gth_subdomains_class = Keypair::new();
    ...
    let (name_account_key, _) = get_seeds_and_key(
        &program_id,
        hashed_name.clone(),
        Some(&gth_subdomains_class.pubkey()),
    ...
    sign_send_instruction(
        ...
        vec![&gth_subdomains_class, &owner],
    )
    .await
    .unwrap();
    ...
    .unwrap();
    println!("Name Record Header: {:?}", name_record_header);
    println!("SOl class {:?}", gth_subdomains_class.pubkey());
    ...
    let update_instruction = update(
        ...
        data,
        name_account_key,
        gth_subdomains_class.pubkey(),
        Some(name_record_header.parent_name),
    )
    .unwrap();
    sign_send_instruction(&mut ctx, update_instruction, vec![&gth_subdomains_class])
        .await
        .unwrap();
    ...
    sign_send_instruction(
        ...
        vec![&owner, &gth_subdomains_class],
    )
    .await
    .unwrap();
